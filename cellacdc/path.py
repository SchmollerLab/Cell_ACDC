...
def listdir(path):
    return natsorted(
        [
            ...
            if not f.startswith(".") and not f == "desktop.ini" and not f == "recovery"
        ]
    )
...
def copy_or_move_tree(
    src: os.PathLike, dst: os.PathLike, copy=False, sigInitPbar=None, sigUpdatePbar=None
):
