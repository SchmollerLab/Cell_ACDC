...
last_entries_metadata_path = os.path.join(
    settings_folderpath, "last_entries_metadata.csv"
)
last_selected_measurements_ini_path = os.path.join(
    settings_folderpath, "last_selected_measurements.ini"
)
channel_file_formats = ("_aligned.h5", ".h5", "_aligned.npz", ".tif")
...
def migrate_models_paths(dst_path):
    ...
    for model in models:
        ...
        weight_location_txt_path = os.path.join(model_path, "weights_location_path.txt")
...
def add_configPars_metrics(configPars_ref, configPars2_to_add):
    ...
    keep_user_path_equations = {
        key: val
        for key, val in configPars_ref["user_path_equations"].items()
...
def save_to_h5(dst_filepath, data):
    ...
    with h5py.File(tempFilepath, "w") as h5f:
        dataset = h5f.create_dataset(
            "data", data.shape, dtype=data.dtype, chunks=chunks, shuffle=False
        )
        dataset[:] = data
...
def load_segm_file(images_path, end_name_segm_file="segm", return_path=False):
    ...
    try:
        ...
        if (
            str(e).find("[Errno 22] Invalid argument") != -1
            and segm_filepath.find("OneDrive") != -1
        ):
            print(traceback.print_exc())
            raise OSError(
                "If the file is online only, and syncing is disabled, this file cannot be accessed."
            )
        else:
            raise e
...
def _add_will_divide_column(acdc_df):
    ...
    for budID, bud_cca_df in cca_df_buds.groupby("Cell_ID"):
        ...
        mothMask = (cca_df["Cell_ID"] == mothID) & (
...
def _fix_corrected_assignment_i(acdc_df: pd.DataFrame):
    ...
    if "corrected_on_frame_i" in acdc_df.columns:
        if (acdc_df["corrected_on_frame_i"] > -1).any():
            acdc_df = acdc_df.drop(columns="corrected_assignment", errors="ignore")
            return acdc_df
...
def _fix_will_divide(acdc_df):
    ...
    IDs_will_divide_wrong = cca_functions.get_IDs_gen_num_will_divide_wrong(cca_df)
...
def read_acdc_df_csv(acdc_df_filepath, index_col=None):
    acdc_df = pd.read_csv(acdc_df_filepath, dtype=acdc_df_str_cols, index_col=index_col)
    return acdc_df
...
def load_acdc_df_file(
    images_path, end_name_acdc_df_file="acdc_output", return_path=False
):
...
def save_acdc_df_file(
    acdc_df, csv_path, custom_annot_columns=None, last_cca_frame_i=None
):
...
def store_copy_acdc_df(posData, acdc_output_csv_path, log_func=printl):
    try:
        ...
        df = pd.read_csv(acdc_output_csv_path, dtype=acdc_df_str_cols).set_index(
            ["frame_i", "Cell_ID"]
        )
    ...
    with zipfile.ZipFile(zip_path, mode="r") as zip:
        for csv_name in csv_names:
            with warnings.catch_warnings():
                ...
                acdc_df = pd.read_csv(zip.open(csv_name), dtype=acdc_df_str_cols)
            acdc_df = _ensure_acdc_df_latest_compatibility(acdc_df)
...
def _store_acdc_df_archive(zip_path, acdc_df_to_store):
    ...
    _copy_acdc_dfs_to_temp_archive(zip_path, temp_zip_path, csv_names, compression_opts)
...
def get_segm_endnames_from_exp_path(exp_path, pos_foldernames=None):
    ...
    for p, pos in enumerate(pos_foldernames):
        ...
        _existingEndnames = get_endnames(_posData.basename, found_files)
...
def get_endname_from_channels(filename, channels):
    ...
    for ch in channels:
        ...
        m = re.search(rf"{ch}(.\w+)*$", filename)
        m_aligned = re.search(rf"{ch_aligned}(.\w+)*$", filename)
...
def pd_bool_and_float_to_int_to_str(
    ...
    if colsToCastInt is None:
        ...
        additional_sister_cols = {col: int for col in additional_sister_cols}
    ...
    for col in colsToCastInt:
        try:
            ...
            acdc_df[col] = (
                acdc_df[col].astype(float).fillna(0).astype(int).astype("string")
            )
...
def get_posData_metadata(images_path, basename):
    ...
    for file in myutils.listdir(images_path):
        ...
        df_metadata = pd.DataFrame(columns=["Description", "values"]).set_index(
            "Description"
        )
...
class loadData:
    def __init__(
        self, imgPath, user_ch_name, relPathDepth=3, QParent=None, log_func=None
    ):
        self.fluo_data_dict = {}
    ...
    def addYXcentroidColsIfMissing(self, show_progress=False):
        ...
        if show_progress:
            pbar = tqdm(
                total=last_frame_i + 1, desc="Adding centroid columns to acdc_df"
            )
    ...
    def getBasenameAndChNames(self, useExt=None, qparent=None):
        ...
        for file in ls:
            ...
            if qparent is not None:
                ...
                html_error_text = (
                    html_error_text.replace(f'"{file}"', f"<code>{file}</code>")
                    .replace(f'"{self.basename}"', f"<code>{self.basename}</code>")
                    .replace(
    ...
    def init_segmInfo_df(self):
        if self.SizeZ > 1 and self.segmInfo_df is not None:
            if "z_slice_used_gui" not in self.segmInfo_df.columns:
                self.segmInfo_df["z_slice_used_gui"] = self.segmInfo_df[
                    "z_slice_used_dataPrep"
                ]
            if "which_z_proj_gui" not in self.segmInfo_df.columns:
                self.segmInfo_df["which_z_proj_gui"] = self.segmInfo_df["which_z_proj"]
            self.segmInfo_df["resegmented_in_gui"] = False
            self.segmInfo_df.to_csv(self.segmInfo_df_csv_path)

        NO_segmInfo = (
            self.segmInfo_df is None or self.filename not in self.segmInfo_df.index
        )
    ...
    def countObjectsInSegmSnapshots(self, categories: set[str] | list[str]):
        ...
        mapper = {"In current position": numObjs}
    ...
    def saveObjCounts(self, countMapper: dict[str, int]):
        ...
        dfCountEndname = segmEndname.replace("segm", "acdc_objects_count").replace(
            ".npz", ".csv"
        )
    ...
    def getSamEmbeddingsPath(self):
        sam_embed_filename = f"{self.basename}_{self.user_ch_name}_sam_embeddings.pt"
    ...
    def loadOtherFiles(
        ...
        for file in ls:
            ...
            loadMetadata = (
                load_metadata
                and file.endswith("metadata.csv")
            ...
            if load_segm_data and is_segm_file and not create_new_segm:
                ...
                if "Unnamed: 0" in df.columns and "Unnamed: 1" in df.columns:
                    df = df.rename(
                        columns={"Unnamed: 0": "filename", "Unnamed: 1": "frame_i"}
                    )
            ...
            elif load_customCombineMetrics and file.endswith(
                "custom_combine_metrics.ini"
            ):
        ...
        if load_last_tracked_i:
            ...
            try:
                self.last_tracked_i = max(
                    self.acdc_df.index.get_level_values(0), default=None
                )
    ...
    def loadMostRecentUnsavedAcdcDf(self):
        ...
        self.acdc_df_found = True
        self.last_tracked_i = max(self.acdc_df.index.get_level_values(0), default=None)
    ...
    def fromTrackerToAcdcDf(self, tracker, tracked_video, save=False, start_frame_i=0):
    ...
    def extractMetadata(self):
        ...
        if "TimeIncrement" in self.metadata_df.index:
            self.TimeIncrement = float(self.metadata_df.at["TimeIncrement", "values"])
        ...
        if "PhysicalSizeX" in self.metadata_df.index:
            self.PhysicalSizeX = float(self.metadata_df.at["PhysicalSizeX", "values"])
        ...
        if "PhysicalSizeY" in self.metadata_df.index:
            self.PhysicalSizeY = float(self.metadata_df.at["PhysicalSizeY", "values"])
        ...
        if "PhysicalSizeZ" in self.metadata_df.index:
            self.PhysicalSizeZ = float(self.metadata_df.at["PhysicalSizeZ", "values"])
        ...
        if "LensNA" in self.metadata_df.index:
            self.numAperture = float(self.metadata_df.at["LensNA", "values"])
        else:
    ...
    def setNotFoundData(self):
        ...
        if self.bkgrDataExists:
            ...
            self.bkgrROIs = []
        if (
            self.dataPrep_ROIcoordsFound is not None
            and not self.dataPrep_ROIcoordsFound
        ):
    ...
    def preprocessedDataArray(self, check_integrity=True):
        ...
        for frame_i, raw_img in enumerate(self.img_data):
            ...
            if preprocess_img is None:
                if check_integrity:
                    raise TypeError("Not all frames have been processed.")
                else:
                    continue
    ...
    def setCombineMetricsConfig(self, ini_path=""):
        ...
        for chName in self.chNames:
            chName_equations = measurements.get_user_combine_metrics_equations(chName)
            chName_equations = {
                key: val
                for key, val in chName_equations.items()
    ...
    def getSegmentedChannelHyperparams(self):
        ...
        if os.path.exists(self.segm_hyperparams_ini_path):
            ...
            channel_name = cp.get(metadata_section, option, fallback=self.user_ch_name)
    ...
    def saveSegmHyperparams(
        ...
        if preproc_recipe is not None:
            preproc_ini_items = config.preprocess_recipe_to_ini_items(preproc_recipe)
            for preproc_section, section_items in preproc_ini_items.items():
                segm_preproc_section = f"{segmEndName}.{preproc_section}"
                segm_preproc_section = f"{segm_preproc_section}.run_number_{run_num}"
    ...
    def setTempPaths(self, createFolder=True):
        ...
        unsaved_acdc_df_filename = acdc_df_filename.replace(".csv", "_autosave.zip")
    ...
    def buildPaths(self):
        ...
        self.sam_embeddings_path = f"{base_path}{self.user_ch_name}_sam_embeddings.pt"
    ...
    def askInputMetadata(
        ...
        metadataWin = apps.QDialogMetadata(
            ...
            parent=self.parent,
            font=apps.font,
            imgDataShape=self.img_data_shape,
            posData=self,
            singlePos=singlePos,
            askSegm3D=askSegm3D,
            additionalValues=self._additionalMetadataValues,
            forceEnableAskSegm3D=forceEnableAskSegm3D,
            SizeT_metadata=SizeT_metadata,
            SizeZ_metadata=SizeZ_metadata,
            basename=basename,
        )
        metadataWin.exec_()
    ...
    def metadataToCsv(self, signals=None, mutex=None, waitCond=None):
        try:
            ...
            if signals is None:
                ...
                msg.warning(self, "Permission denied", permissionErrorTxt)
                self.metadata_df.to_csv(self.metadata_csv_path)
            else:
                mutex.lock()
    ...
    def saveMetadata(
        self, signals=None, mutex=None, waitCond=None, additionalMetadata=None
    ):
        segmEndName = self.getSegmEndname()
        isSegm3Dkey = f"{segmEndName}_isSegm3D"
        if self.metadata_df is None:
    ...
    def saveTrackedLostCentroids(
        self, tracked_lost_centroids_list=None, _tracked_lost_centroids_list=None
    ):

        if not (
            self.tracked_lost_centroids
            or tracked_lost_centroids_list
            or _tracked_lost_centroids_list
        ):
            return
        ...
        if _tracked_lost_centroids_list is not None:
            ...
            tracked_lost_centroids_list = {
                k: v for k, v in tracked_lost_centroids_list.items()
            }

        else:
            tracked_lost_centroids_list = {
                k: list(v) for k, v in self.tracked_lost_centroids.items()
            }
        ...
        try:
            ...
            self.saveTrackedLostCentroids(
                _tracked_lost_centroids_list=tracked_lost_centroids_list
            )
    ...
    def loadTrackedLostCentroids(self):
        try:
            with open(self.tracked_lost_centroids_json_path, "r") as json_file:
                ...
                self.tracked_lost_centroids = {
                    int(k): {tuple(int(val) for val in centroid) for centroid in v}
                    for k, v in tracked_lost_centroids_list.items()
                }
    ...
    def get_values_cca(self, exp_path):
        ...
        for pos in pos_foldernames:
            ...
            if os.path.isdir(pos_path):
                ...
                if cc_stage_found:
                    values.append(
                        f"{pos} (Last analyzed frame: {last_analyzed_frame_i})"
                    )
                else:
                    values.append(pos)
        self.values = values
...
def autoLineBreak(
    text, length
):  # automatic line breaking for tooltips. Keeps indentation with spaces and preexisting line breaks
    lines = []
    current_line = []
...
def format_bullet_points(
    text,
):  # indentation for bullet points in tooltips. Implementation not robust
    lines = text.split("\n")
    formatted_lines = []
    indent = False
    indentNo = 0
...
def format_number_list(
    text,
):  # indentation for number points in tooltips. Implementation not robust
    lines = text.split("\n")
    formatted_lines = []
    indent = False
    indentNo = 0
...
def askOpenCsvFile(title="Open CSV file", start_dir=None, qparent=None):
    ...
    args = (qparent, title, start_dir, file_types)
