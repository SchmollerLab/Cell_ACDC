...
def custom_metrics_desc(
    isZstack, chName, posData=None, isSegm3D=False, return_combine=False
):
    how_3Dto2D, how_3Dto2D_desc = get_how_3Dto2D(isZstack, isSegm3D)
...
def standard_metrics_desc(
    ...
    how_3Dto2D, how_3Dto2D_desc = get_how_3Dto2D(isZstack, isSegm3D)
    metrics_names = _get_metrics_names(is_manual_bkgr_present=isManualBackgrPresent)
    bkgr_val_names = _get_bkgr_val_names(is_manual_bkgr_present=isManualBackgrPresent)
    metrics_desc = {}
    bkgr_val_desc = {}
    for how, how_desc in zip(how_3Dto2D, how_3Dto2D_desc):
...
def get_conc_keys(amount_colname):
    conc_key_vox = re.sub(
        r"amount_([A-Za-z]+)", r"concentration_\1_from_vol_vox", amount_colname
    )
        ...
        for ch in channels:
            if col.startswith(f"{ch}_"):
                ...
                if col.find("_bkgrVal_") != -1:
                    ...
                    # Foregr metric
                    is_standard = any([col.find(f"_{f}") != -1 for f in standard_funcs])
                    if is_standard:
                        # Standard metric
...
def _is_numeric_dtype(dtype):
    is_numeric = dtype is float or dtype is int
    return is_numeric
...
def get_autoBkgr_mask(lab, isSegm3D, posData, frame_i):
    autoBkgr_mask = lab == 0
    autoBkgr_mask = _mask_0valued_pixels_from_alignment(autoBkgr_mask, frame_i, posData)
    if isSegm3D:
...
def get_obj_size_metric(col_name, obj, isSegm3D, yx_pxl_to_um2, vox_to_fl_3D):
...
def get_foregr_obj_array(foregr_arr, obj, isSegm3D, z_slice=None, how=None):
    if foregr_arr.ndim == 3 and isSegm3D:
        ...
        # 3D mask on 2D data
        use_proj = z_slice is None or how is None or how != "zSlice"
...
def get_bkgr_data(
    ...
    """Auto Background"""
    bkgr_data["autoBkgr"] = {"": 0, "maxProj": 0, "meanProj": 0, "zSlice": 0, "3D": 0}
    if isZstack:
        if isSegm3D:
...
def add_size_metrics(
    ...
    for o, obj in enumerate(tqdm(rp, ncols=100, leave=False)):
        for col in size_metrics_to_save:
            val = get_obj_size_metric(col, obj, isSegm3D, yx_pxl_to_um2, vox_to_fl_3D)
...
def add_bkgr_values(
    ...
    for col, (bkgr_type, func_name, how) in bkgr_metrics_params.items():
        ...
        if bkgr_type == "manualBkgr":
            add_manual_bkgr_values(manualBackgrRp, foregr_data, df, col, how, bkgr_func)
            continue
...
def get_channel_indipend_custom_metrics_params(
    ...
    for col in ch_indipend_custom_metric_cols:
        for metric, custom_func in ch_indipend_custom_func_dict.items():
            custom_pattern = rf"({metric})_?({how_3D_to_2D_pattern}*)"
...
def get_metrics_params(all_channels_metrics, metrics_func, custom_func_dict):
    ...
    az = r"[A-Za-z0-9]"
    bkgrVal_pattern = rf"_({az}+)_bkgrVal_({az}+)_?({az}*)$"
    ...
    for channel_name, columns in all_channels_metrics.items():
        for col in columns:
            ...
            if m:
                ...
                bkgr_metrics_params[channel_name][col] = (bkgr_type, func_name, how)
                continue
    ...
    for col in existing_colnames:
        for selected_prop in selected_props_names:
            if selected_prop == col:
                ...
                continue
            m = re.match(rf"{selected_prop}-\d", col)
            if m is not None:
                selected_rp_cols.append(col)
...
def calc_circularity(obj):
    if obj.image.ndim == 3:
        raise TypeError("Circularity can only be calculated for 2D objects.")
...
def calc_roundness(obj):
    if obj.image.ndim == 3:
        raise TypeError("Roundness can only be calculated for 2D objects.")
