...
* **Hull contour (** |hullContToolButton| **"K"):** Right-click on a cell to replace it with its hull contour. Use it to fill cracks and holes. When working with 3D segmentation masks, the default behaviour is to replace only the viewed z-slice. When working with 3D segmentation masks, the default behaviour is to apply to the entire 3D object. When viewing a 3D object z-slice, hold "Shift" while right-clicking to apply only to the viewed z-slice.
