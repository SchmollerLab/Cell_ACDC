...
def make_iformat_writer_class(class_name):
    ...
    class IFormatWriter(object):
        ...
        getColorModel = jutil.make_method(
            "getColorModel", "()Ljava/awt/image/ColorModel;", "Gets the color model."
        )
        ...
        getPixelTypes = jutil.make_method(
            "getPixelTypes", "()[I", "Gets the supported pixel types."
        )
        ...
        isSupportedType = jutil.make_method(
            "isSupportedType", "(I)Z", "Checks if the given pixel type is supported."
        )
        saveBytes = jutil.make_method(
            "saveBytes", "([BZ)V", "Saves the given byte array to the current file."
        )
        saveBytesIB = jutil.make_method(
            "saveBytes", "(I[B)V", "Saves bytes, first arg is image #"
        )
        #        saveBytes = jutil.make_method('saveBytes', '([BIZZ)V',
        #                                      'Saves the given byte array to the given series in the current file.')
        savePlane = jutil.make_method(
            "savePlane",
        ...
        setColorModel = jutil.make_method(
            "setColorModel", "(Ljava/awt/image/ColorModel;)V", "Sets the color model."
        )
        ...
        setValidBitsPerPixel = jutil.make_method(
            "setValidBitsPerPixel", "(I)V", "Sets the number of valid bits per pixel"
        )
...
def make_image_writer_class():
    ...
    #
    class_list = jutil.make_instance(
    ...
    class ImageWriter(IFormatWriter):
        ...
        setId = jutil.make_method(
            "setId", "(Ljava/lang/String;)V", "Sets the current file name."
        )
        addStatusListener = jutil.make_method(
            "addStatusListener",
            "()Lloci/formats/StatusListener;",
            "Adds a listener for status update events.",
        )
        close = jutil.make_method(
            "close", "()V", "Closes currently open file(s) and frees allocated memory."
        )
        getFormat = jutil.make_method(
            "getFormat", "()Ljava/lang/String;", "Gets the name of this file format."
        )
...
def make_format_writer_class(class_name):
    ...
    """
    new_fn = jutil.make_new(class_name, "(Ljava/lang/String;Ljava/lang/String;)V")

    class FormatWriter(object):
        __doc__ = (
            """A wrapper for %s implementing loci.formats.FormatWriter
        See http://hudson.openmicroscopy.org.uk/job/LOCI/javadoc/loci/formats/FormatWriter"""
            % class_name
        )
...
def getRGBColorSpace():
    ...
    cs_linear_rgb = jutil.get_static_field(
        "java/awt/color/ColorSpace", "CS_LINEAR_RGB", "I"
    )
...
def getColorModel(
    ...
    jtransfer_type = jutil.get_static_field(
        "java/awt/image/DataBuffer", transfer_type, "I"
    )
...
if __name__ == "__main__":
    ...
    filename = (
        "/Users/afraser/Desktop/cpa_example/images/AS_09125_050116000001_A01f00d0.png"
    )
    ...
    for image in images:
        if len(image.shape) == 3 and image.shape[2] == 3:
            save_im = (
                np.array([image[:, :, 0], image[:, :, 1], image[:, :, 2]])
                .astype(np.uint8)
                .flatten()
            )
        else:
