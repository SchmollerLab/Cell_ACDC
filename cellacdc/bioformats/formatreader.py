...
def make_iformat_reader_class():
    ...
    class IFormatReader(object):
        ...
        close = jutil.make_method(
            "close", "()V", "Close the currently open file and free memory"
        )
        ...
        getGlobalMetadata = jutil.make_method(
            "getGlobalMetadata",
            "()Ljava/util/Hashtable;",
            "Obtains the hashtable containing the global metadata field/value pairs",
        )
        getMetadata = getGlobalMetadata
        getMetadataValue = jutil.make_method(
            "getMetadataValue",
            "(Ljava/lang/String;)Ljava/lang/Object;",
            "Look up a specific metadata value from the store",
        )
        getSeriesMetadata = jutil.make_method(
            "getSeriesMetadata",
            "()Ljava/util/Hashtable;",
            "Obtains the hashtable contaning the series metadata field/value pairs",
        )
        getSeriesCount = jutil.make_method(
            "getSeriesCount", "()I", "Return the # of image series in the file"
        )
        getSeries = jutil.make_method(
            "getSeries", "()I", "Return the currently selected image series"
        )
        getImageCount = jutil.make_method(
            "getImageCount",
            "()I",
            "Determines the number of images in the current file",
        )
        getIndex = jutil.make_method(
            "getIndex", "(III)I", "Get the plane index given z, c, t"
        )
        getRGBChannelCount = jutil.make_method(
            "getRGBChannelCount",
            "()I",
            "Gets the number of channels per RGB image (if not RGB, this returns 1",
        )
        getSizeC = jutil.make_method(
            "getSizeC", "()I", "Get the number of color planes"
        )
        getSizeT = jutil.make_method(
            "getSizeT", "()I", "Get the number of frames in the image"
        )
        getSizeX = jutil.make_method("getSizeX", "()I", "Get the image width")
        getSizeY = jutil.make_method("getSizeY", "()I", "Get the image height")
        getSizeZ = jutil.make_method("getSizeZ", "()I", "Get the image depth")
        getPixelType = jutil.make_method(
            "getPixelType", "()I", "Get the pixel type: see FormatTools for types"
        )
        isLittleEndian = jutil.make_method(
            "isLittleEndian", "()Z", "Return True if the data is in little endian order"
        )
        isRGB = jutil.make_method(
            "isRGB", "()Z", "Return True if images in the file are RGB"
        )
        ...
        isIndexed = jutil.make_method(
            "isIndexed",
            "()Z",
            "Return True if the raw data is indexes in a lookup table",
        )
        openBytes = jutil.make_method(
            "openBytes", "(I)[B", "Get the specified image plane as a byte array"
        )
        openBytesXYWH = jutil.make_method(
            "openBytes",
            "(IIIII)[B",
        ...
        setSeries = jutil.make_method(
            "setSeries", "(I)V", "Set the currently selected image series"
        )
        setGroupFiles = jutil.make_method(
            "setGroupFiles",
            "(Z)V",
            "Force reader to group or not to group files in a multi-file set",
        )
        setMetadataStore = jutil.make_method(
            "setMetadataStore",
            "(Lloci/formats/meta/MetadataStore;)V",
            "Sets the default metadata store for this reader.",
        )
        setMetadataOptions = jutil.make_method(
            "setMetadataOptions",
        ...
        def setId(self, path):
            ...
            jutil.call(self.o, "setId", "(Ljava/lang/String;)V", path)

        getMetadataStore = jutil.make_method(
            "getMetadataStore",
            "()Lloci/formats/meta/MetadataStore;",
            "Retrieves the current metadata store for this reader.",
        )
        get8BitLookupTable = jutil.make_method(
            "get8BitLookupTable", "()[[B", "Get a lookup table for 8-bit indexed images"
        )
        get16BitLookupTable = jutil.make_method(
            "get16BitLookupTable",
            "()[[S",
            "Get a lookup table for 16-bit indexed images",
        )

        def get_class_name(self):
            return jutil.call(
                jutil.call(self.o, "getClass", "()Ljava/lang/Class;"),
                "getName",
                "()Ljava/lang/String;",
            )
...
def make_reader_wrapper_class(class_name):
    ...
    class ReaderWrapper(IFormatReader):
        ...
        setId = jutil.make_method(
            "setId", "(Ljava/lang/String;)V", "Set the name of the data file"
        )
...
def omero_login():
    if __omero_config_file is not None and os.path.isfile(__omero_config_file):
        ...
        [
            x is not None
            for x in (__omero_server, __omero_port, __omero_username, __omero_password)
        ]
    ):
        set_omero_credentials(
...
def load_using_bioformats_url(
    ...
    with ImageReader(url=url) as rdr:
        return rdr.read(
            c, z, t, series, index, rescale, wants_max_intensity, channel_names
        )
...
class ImageReader(object):
    ...
    def __init__(self, path=None, url=None, perform_init=True):
        ...
        if not os.path.isfile(self.path):
            raise IOError(errno.ENOENT, 'The file, "%s", does not exist.' % path, path)

        self.stream = jutil.make_instance(
            "loci/common/RandomAccessInputStream", "(Ljava/lang/String;)V", self.path
        )
    ...
    def read(
        ...
        if XYWH is not None:
            ...
            openBytes_func = lambda x: self.rdr.openBytesXYWH(
...
def get_image_reader(key, path=None, url=None):
    ...
    if (path, url) in __image_reader_cache:
        ...
        if (
            OMERO_READER_IMPORTED
            and OMERO_IMPORTED
            and omero_reader_enabled()
            and url is not None
            and url.lower().startswith("omero:")
        ):
