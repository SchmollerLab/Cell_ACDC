...
from ._types import ChannelsDict
    ...
    if labels.ndim == 3:
        ...
        for z, lab in enumerate(labels):
            _result = post_process_segm_lab2D(
                lab, min_solidity, min_area, max_elongation, return_delIDs=return_delIDs
            )
            if return_delIDs:
                lab, _delIDs = _result
        ...
        result = post_process_segm_lab2D(
            labels, min_solidity, min_area, max_elongation, return_delIDs=return_delIDs
        )
...
def post_process_segm_lab2D(
    lab, min_solidity=None, min_area=None, max_elongation=None, return_delIDs=False
):
    """
...
def track_sub_cell_objects_third_segm_acdc_df(
    ...
    for frame_i, lab in enumerate(track_parent_objs_segm_data):
        ...
        keys.append(frame_i)
    third_segm_acdc_df = pd.concat(dfs, keys=keys, names=["frame_i", "Cell_ID"])
    return third_segm_acdc_df


def track_sub_cell_objects_acdc_df(
    tracked_subobj_segm_data,
    subobj_acdc_df,
    all_old_sub_ids,
    ...
    for frame_i, lab_sub in enumerate(tracked_subobj_segm_data):
        ...
        if subobj_acdc_df is None:
            ...
            if "relative_ID" in sub_acdc_df_frame_i.columns:
                sub_acdc_df_frame_i["relative_ID"] = sub_acdc_df_frame_i[
                    "relative_ID"
                ].replace(old_sub_ids)
...
def track_sub_cell_objects(
    ...
    for frame_i, (lab, lab_sub) in enumerate(segm_data_zip):
        ...
        for sub_obj in rp_sub:
            ...
            intersect_IDs, intersections = np.unique(intersect_mask, return_counts=True)
...
def calc_resolution_limited_vol(
    ...
    zyx_resolution = np.asarray([z_resolution_limit, yx_resolution, yx_resolution])
...
def align_frames_2D(
    data, slices=None, register=True, user_shifts=None, pbar=False, sigPyqt=None
):
    registered_shifts = np.zeros((len(data), 2), int)
    data_aligned = np.copy(data)
...
def get_obj_contours(
    ...
    contours, _ = cv2.findContours(obj_image, retrieveMode, cv2.CHAIN_APPROX_NONE)
...
def smooth_contours(lab, radius=2):
    ...
    for obj in skimage.measure.regionprops(lab):
        ...
        x = np.round(
            skimage.filters.gaussian(x, sigma=sigma, preserve_range=True)
        ).astype(int)
        y = np.round(
            skimage.filters.gaussian(y, sigma=sigma, preserve_range=True)
        ).astype(int)
    ...
    for frame_i, tracked_frame_labels in enumerate(tracked_labels):
        for tracked_label in tracked_frame_labels:
            ...
            if not parent_label:
                ...
                sister_label == "1" and f"{parent_label}_0" not in tracked_frame_labels
            ):
...
def annotate_lineage_tree_from_labels(tracked_labels, labels_to_IDs_mapper):
    IDs_to_labels_mapper = {ID: label for label, ID in labels_to_IDs_mapper.items()}
    cca_dfs = []
    keys = []
...
def apply_trackedIDs_mapper_to_acdc_df(tracked_IDs_mapper, deleted_IDs_mapper, acdc_df):
    acdc_dfs_renamed = []
...
def add_cca_info_from_parentID_col(
    ...
    for frame_i, df_frame in iterable:
        ...
        for newID in newIDs:
            ...
            if parentGenNum is not None:
                prentGenNumTree = prev_acdc_df.at[parentID, "generation_num_tree"]
                newGenNumTree = prentGenNumTree + 1
                parentRootID = prev_acdc_df.at[parentID, "root_ID_tree"]
                ...
                sisterIDmask = (df_frame[parentID_colname] == parentID) & (
    ...
    if acdc_dfs:
        acdc_df_with_cca = pd.concat(acdc_dfs, keys=keys, names=["frame_i", "Cell_ID"])
...
class LineageTree:
    ...
    def _normalize_gen_num(self, acdc_df):
        ...
        normalizing_df.loc[df_emerg_moth_uknown.index, "gen_num_diff"] = df_diff
    ...
    def _build_tree(self, gen_df, ID):
        ...
        if self.is_new_tree:
            try:
                gen_num_relID_tree = (
                    self.df_G1.at[(start_frame_i, relID), "generation_num_tree"] - 1
                )
            except Exception as e:
    ...
    def add_lineage_tree_table_to_acdc_df(self):
        ...
        for frame_i in frames_idx:
            ...
            for ID in IDs:
                ...
                df_tree_iter = self.df_G1.groupby(
                    ["Cell_ID", "generation_num"], group_keys=False
    ...
    def _build_tree_S(self, cols_tree):
        ...
        gen_acdc_df = (
            self.acdc_df.reset_index()
            .set_index(["Cell_ID", "generation_num", "cell_cycle_stage"])
            .sort_index()
        )
    ...
    def to_arboretum(self, rebuild=False):
        ...
        graph = {
            ...
            for child_ID, parent_ID in zip(
                graph_df.Cell_ID_tree, graph_df.parent_ID_tree
            )
        }

        properties = pd.DataFrame(
            {"t": df.frame_i, "root": df.root_ID_tree, "parent": df.parent_ID_tree}
        )
...
def preprocess_multi_pos_from_recipe(
    image_data: Iterable[np.ndarray], recipe: List[Dict[str, Any]]
):
    pbar = tqdm(total=len(image_data), unit="Position", ncols=100)
    preprocessed_data = []
    for pos_i, image in enumerate(image_data):
        preprocessed_image = preprocess_zstack_from_recipe(image, recipe, pbar_pos=1)
...
def preprocess_video_from_recipe(image, recipe: List[Dict[str, Any]], pbar_pos=0):
    if image.ndim < 3:
        raise TypeError(
            f"Only 3D or 4D videos allowed. Input image has {image.ndim} dimensions!"
        )
            ...
            for frame_i, frame_img in enumerate(preprocessed_image):
                if frame_img.ndim == 3:
                    ...
                    if preprocessed_img.dtype != preprocessed_image.dtype:
                        preprocessed_image = preprocessed_image.astype(
                            preprocessed_img.dtype
                        )
                    preprocessed_image[frame_i] = preprocessed_img
                else:
                    preprocessed_img = preprocess_image_from_recipe(frame_img, (step,))
                    if preprocessed_img.dtype != preprocessed_image.dtype:
                        preprocessed_image = preprocessed_image.astype(
                            preprocessed_img.dtype
                        )
...
def preprocess_zstack_from_recipe(image, recipe: List[Dict[str, Any]], pbar_pos=0):
    if image.ndim != 3:
        raise TypeError(
            f"Only 3D z-stack images allowed. Input image has {image.ndim} dimensions!"
        )
...
def segm_model_segment(
    ...
    frame_i=None,
    preproc_recipe=None,
    is_timelapse_model_and_data=False,
    posData=None,
    start_z_slice=0,
):
    ...
    if is_timelapse_model_and_data:
        model_kwargs = pop_signals_kwarg_if_not_needed(model.segment3DT, model_kwargs)
        segm_data = model.segment3DT(image, **model_kwargs)
        return segm_data

    model_kwargs = pop_signals_kwarg_if_not_needed(model.segment, model_kwargs)
    ...
    try:
        lab = model.segment(image, frame_i=frame_i, posData=posData, **model_kwargs)
        return lab
    except TypeError as err:
        if str(err).find("unexpected keyword argument") == -1:
            # Raise error since it's not about the missing posData kwarg
    ...
    try:
        lab = model.segment(image, frame_i=frame_i, **model_kwargs)
        return lab
    except TypeError as err:
        pass
...
def tracker_track(
    segm_data, tracker, track_params, intensity_img=None, logger_func=print
):
    ...
    for args, kwarg_to_remove in product(args_to_try, kwargs_to_remove):
        ...
        try:
            ...
            is_unexpected_kwarg = (
                str(err).find("got an unexpected keyword argument 'signals'") != -1
            )
            is_missing_arg = (
                str(err).find("missing 1 required positional argument:") != -1
            )
...
class CcaIntegrityChecker:
    ...
    def get_mother_IDs_with_multiple_buds(self):
        ...
        mother_IDs_with_multiple_buds = mothers_of_buds[mothers_of_buds.duplicated()]
        return mother_IDs_with_multiple_buds.values

    def get_IDs_cycles_without_G1(self, global_cca_df):
        global_cca_df_moths_hist_known = global_cca_df[
        ...
        G1_not_present_mask = grouped_cycles["cell_cycle_stage"].agg(
            lambda x: ~x.eq("G1").any()
        )
        IDs_cycles_without_G1 = G1_not_present_mask[G1_not_present_mask].index.to_list()
    ...
    def get_bud_IDs_gen_num_nonzero(self):
        cca_df_S = self.cca_df_S
        cca_df_S_buds = cca_df_S[cca_df_S["relationship"] == "bud"]
        bud_IDs_gen_num_nonzero = cca_df_S_buds[
            cca_df_S_buds["generation_num"] != 0
        ].index.to_list()
    ...
    def get_moth_IDs_gen_num_non_greater_one(self):
        ...
        moth_IDs_gen_num_non_greater_one = cca_df_S_moths[
            cca_df_S_moths["generation_num"] < 1
        ].index.to_list()
    ...
    def get_buds_G1(self):
        cca_df_S = self.cca_df_S
        cca_df_S_buds = cca_df_S[cca_df_S["relationship"] == "bud"]
        buds_G1 = cca_df_S_buds[
            cca_df_S_buds["cell_cycle_stage"] == "G1"
        ].index.to_list()
        return buds_G1

    def get_cell_S_rel_ID_zero(self):
        cca_df_S = self.cca_df_S
        cell_S_rel_ID_zero = cca_df_S[cca_df_S["relative_ID"] < 1].index.to_list()
...
def cellpose_v3_run_denoise(
    ...
    denoised_img = denoise_model.run(
        image, timelapse=timelapse, isZstack=isZstack, **run_params
    )  # may have to give rgb stuff too!
    return denoised_img
...
def fucci_pipeline_executor_map(input, **filter_kwargs):
    ...
    ch1_img = skimage.exposure.rescale_intensity(ch1_img, out_range=(0, 0.5))
    ch2_img = skimage.exposure.rescale_intensity(ch2_img, out_range=(0, 0.5))
...
def preprocess_image_from_recipe_multithread(
    image: np.ndarray, recipe: List[Dict[str, Any]], n_threads: int = None
):
    preprocessed_image = image
    for step in recipe:
        method = step["method"]
        ...
        if is_func_time_capable:
            ...
            with ThreadPoolExecutor(max_workers=n_threads) as executor:
                ...
                func = partial(preprocess_exceutor_map, recipe=(step,))
...
def combine_channels_multithread(
    ...
    with ThreadPoolExecutor(max_workers=n_threads) as executor:
        if signals:
            ...
            pbar = tqdm(total=len(images_paths), ncols=100, desc="Combining channels")
...
def _get_img_from_data_key(data, key, num_dim, seg=False):
    ...
    if num_dim == 3:  # t x y
        ...
        raise ValueError(f"Invalid number of dimensions in img_data. {num_dim}")
...
def _add_missing_dims(segm, target_shape, use_broadcast=False):
    ...
    raise ValueError(f"Invalid shape. segm: {segm.shape}, target: {target_shape}")
...
def _verify_shape_ndim(img_data, target_dims, target_shape, is_segm=False):
    ...
    if img_data.ndim == target_dims:
        ...
        _shape_mismatch_error(
            [(i, i) for i in range(target_dims)], img_data, target_shape
        )
...
def combine_channels_func(
    ...
    if not save_filepath and not return_img:
        raise ValueError(
            "Either save_filepath must be provided or return_img must be true"
        )
    ...
    if provided not in (0, 3):
        raise ValueError(
            "return_img, data, and key must all be provided together or not at all"
        )
    ...
    if data is None:
        for channel in fluo_channel_names:
            ch_filepath = load.get_filepath_from_endname(images_path, channel)
        ...
        #     n_dim -= 1
        is_2D_segm_on_3D = (
            posData.SizeZ != 1 and posData.allData_li[0]["labels"].ndim == 2
        )
        ...
        for channel in segm_channels:
            channel_img_data = _get_img_from_data_key(
                segm_data_dict[channel], key, n_dim, seg=True
            )
            if original_dtype is None:
                original_dtype = channel_img_data.dtype
        ...
        if current_segm:  # here we dont need to get/appply target dim, as we already ignore z slice key if segm is 2D and image 3D (time is always treated differently!)
            if (
                posData.SizeT == 1
            ):  # actually t is always there, we only need to subtract for curr. segm
                n_dim -= 1
    ...
    for i, ch in zip(channel_keys, channel_names):
        if ch in fluo_ch_data_list:
            ...
            )  # false since we already expanded
    ...
    for i, step_info in steps.items():
        ...
        steps[i]["channel_data"] = skimage.exposure.rescale_intensity(
            steps[i]["channel_data"], out_range=(norm_min, norm_max)
        )
    ...
    if not output_as_segm:
        output_img = skimage.exposure.rescale_intensity(output_img, out_range=(0, 1))
    ...
    if keep_input_data_type and not output_as_segm:
        try:
            ...
            is_in_bounds = (
                output_img.max() <= dtype_max and output_img.min() >= dtype_min
            )
            if is_in_bounds:
...
def split_segm_masks_mother_bud_line(
    cells_segm_data, segm_data_to_split, acdc_df, debug=False
):
    acdc_df = acdc_df.set_index(["frame_i", "Cell_ID"])
    ...
    for frame_i, lab in enumerate(cells_segm_data):
        ...
        for obj in rp:
            ...
            for z, lab_split in enumerate(segm_data_to_split[frame_i]):
                ...
                split_segm_close[frame_i, z, split_close_yy, split_close_xx] = obj.label

                split_away_yy = lab_split_yy[~close_to_bud_mask]
                split_away_xx = lab_split_xx[~close_to_bud_mask]

                split_segm_away[frame_i, z, split_away_yy, split_away_xx] = obj.label
...
def _compute_all_obj_to_obj_contour_dist_pairs(
    ...
    dist_matrix_df = pd.DataFrame(
        index=IDs, columns=IDs, data=np.full((num_objs, num_objs), np.inf)
    )
    len_longest_contour = np.max([len(contours) for contours in all_contours.values()])
...
def convexity_defects(img, eps_percent):
    ...
    cnt = cv2.approxPolyDP(
        cnt, eps_percent * cv2.arcLength(cnt, True), True
    )  # see https://www.programcreek.com/python/example/89457/cv22.convexityDefects
    hull = cv2.convexHull(
        cnt, returnPoints=False
    )  # see https://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_contours/py_contours_more_functions/py_contours_more_functions.html
...
def split_along_convexity_defects(ID, lab, max_ID, max_i=1, eps_percent=0.01):
    lab_ID_bool = lab == ID
    ...
    sep_bud_label = skimage.measure.label(sep_bud_img, connectivity=2)
...
def validate_multidimensional_recipe(
    recipe: List[Dict[str, Any]], apply_to_all_zslices=False, apply_to_all_frames=False
):
    for step in recipe:
        method = step["method"]
        func = PREPROCESS_MAPPER[method]["function"]
...
def insert_missing_objects(segm_dst, segm_src, is_timelapse=True, display_pbar=True):
    if not is_timelapse:
...
def fix_sparse_directML(verbose=True):
    ...
    def fallback_to_cpu_on_sparse_error(func, verbose=True):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:  # try to perform the operation and move to dml if possible
                result = func(
                    *args, **kwargs
                )  # run function with current args and kwargs
                if device_arg is not None and str(device_arg).lower() == "dml":
                    try:  # try to move result to dml
                        ...
                        if verbose:
                            warnings.warn(
                                f"Sparse op failed on DirectML, falling back to CPU: {e}"
                            )
                        kwargs["device"] = torch.device("cpu")
                        return func(
                            *args, **kwargs
                        )  # try again, after setting device to cpu
                return result  # just return result if all worked well

            except (
                RuntimeError
            ) as e:  # try and run on dlm, if it fails, fallback to cpu
                if "sparse" in str(e).lower() or "not implemented" in str(e).lower():
                    ...
                    kwargs["device"] = torch.device(
                        "cpu"
                    )  # if rutime warning caused by sparse tensor, set device to cpu
    ...
    # High-level API
    torch.sparse_coo_tensor = fallback_to_cpu_on_sparse_error(
        torch.sparse_coo_tensor, verbose=verbose
    )

    # Low-level API
    if hasattr(torch._C, "_sparse_coo_tensor_unsafe"):
        torch._C._sparse_coo_tensor_unsafe = fallback_to_cpu_on_sparse_error(
            torch._C._sparse_coo_tensor_unsafe, verbose=verbose
        )

    if hasattr(torch._C, "_sparse_coo_tensor_with_dims_and_tensors"):
        torch._C._sparse_coo_tensor_with_dims_and_tensors = (
            fallback_to_cpu_on_sparse_error(
                torch._C._sparse_coo_tensor_with_dims_and_tensors, verbose=verbose
            )
        )

    if hasattr(torch.sparse, "SparseTensor"):
        torch.sparse.SparseTensor = fallback_to_cpu_on_sparse_error(
            torch.sparse.SparseTensor, verbose=verbose
        )
    ...
    **kwargs : dict
    ...
    if iter_axis is None:
        ...
        if benchmark:
            t1 = time.perf_counter()
            printl(
                f"Processing time: {(t1 - t0) * 1000:.2f} ms, no parallel since iter_axis is None"
            )
        return out
    ...
    if parallel:
        ...
        with executor_func() as executor:
            futures = {
                executor.submit(
                    func, image[i_in], *args, frame_index_out=i_out, **kwargs
                )
...
def natsort_acdc_columns(columns: Iterable[str], prepend_default_index_cols=True):
