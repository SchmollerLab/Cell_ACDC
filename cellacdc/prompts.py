...
class select_channel_name:
    ...
    def _get_available_channels_from_metadata(
        ...
        if channelNames:
            ...
            for chName in channelNames:
                ...
                for file in filenames:
                    patterns = (f"{chName}.tif", f"{chName}_aligned.npz")
        ...
        for file in filenames:
            ends = [
                ext
                for ext in channelExt
                if (file.endswith(ext) and not file.endswith("btrack_tracks.h5"))
    ...
    def get_available_channels(
        ...
        for file in filenames:
            ...
            if useExt is None:
                ...
                validFile = (file.find("_acdc_output_") != -1 and ext == ".csv") or (
                    file.find("_segm_") != -1 and ext == ".npz"
                )
            if not validFile:
                continue
        ...
        if self.which_channel is not None:
            ...
            if self.which_channel == "segm":
                is_phase_contr_li = [
                    c.lower().find("phase") != -1 for c in channel_names
                ]
...
def exportToVideoFinished(preferences, conversion_to_mp4_successful, qparent=None):
    from cellacdc import widgets
...
def init_segm_model_params(
    ...
    is_sam_model = model_name in ("segment_anything", "sam2") and check_sam_embeddings
    ...
    if is_sam_model and sam_embeddings_exist and not sam_embeddings_loaded:
        load_sam_embed, only_load_sam_embed, cancel = askSamLoadEmbeddings(
            posData.sam_embeddings_path, qparent=qparent, is_gui_caller=is_gui_caller
        )
        if cancel:
            return out
    ...
    if ask_sam_embeddings:
        sam_only_embeddings, sam_also_embeddings, cancel = askSamSaveEmbeddings(
            qparent=qparent
        )
        if cancel:
            return out
        ...
        win.model_kwargs["save_embeddings"] = sam_only_embeddings or sam_also_embeddings
