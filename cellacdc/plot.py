...
from . import _run, core, utils


def matplotlib_cmap_to_lut(
    cmap: Union[Iterable, matplotlib.colors.Colormap, str], n_colors: int = 256
):
...
def _add_colorbar_axes(
    ax: plt.Axes, im: matplotlib.image.AxesImage, size="5%", pad=0.07, label=""
):
...
def _get_groups_data(
    ...
    if normalize_x:
        min_dx = min([group_df[x].diff().abs().min() for _, group_df in grouped])
        max_n_decimals = 0
...
def _get_heatmap_yticks(nrows, group_height, yticks_labels, group_label_depth):
    yticks = np.arange(0, nrows * group_height, group_height) - 0.5
    # yticks = yticks + group_height/2 - 0.5

    if group_label_depth is not None:
        df_ticks = (
            pd.DataFrame({"yticks": yticks, "yticks_labels": yticks_labels})
            .set_index("yticks")
            .astype(str)
        )
...
def _raise_convert_time_how(convert_time_how):
    print(error_below)
    conversion_methods = [f"    * {how}" for how in _core.time_units_converters.keys()]
    conversion_methods = "\n".join(conversion_methods)
    print(f'"{convert_time_how}" is not a valid `convert_time_how` value.\n')
...
def _get_heatmap_xticks(
    ...
    if x_labels is not None:
        series_xticks = series_xindex[series_xindex.isin(x_labels)].drop_duplicates(
            keep="first"
        )
    else:
...
def heatmap(
    ...
    _add_colorbar_axes(ax, im, size=_size_perc, pad=colorbar_pad, label=colorbar_label)
...
def _binned_mean_stats(x, y, bins, bins_min_count):
    ...
    bin_std, _, _ = scipy.stats.binned_statistic(x, y, statistic=np.nanstd, bins=bins)
...
def texts_to_pg_scatter_symbols(
    texts: Union[str, list], font=None, progress=True, return_scales=False
):
    if font is None:
        font = QtGui.QFont()
        font.setPixelSize(11)
...
def plt_moth_bud_lines(
    ax, cca_df, lab=None, rp=None, plot_kwargs=None, only_moth_IDs=None
):
    if rp is None:
        rp = skimage.measure.regionprops(lab)
