...
def readFilenamePattern(fileName):
    matching_frame_name_pattern, frameNumber = get_frame_num_and_pattern(fileName)
...
def run(mainWin):
    ...
    selectHowWin = apps.QDialogCombobox(
        "Select how files are structured",
        items,
    ...
    mainWin.log(
        f'[Data Re-Struct] Selected file structure = "{selectHowWin.selectedItemText}"'
    )
    ...
    """)
    msg.information(
        mainWin, "Microscopy files location", txt, buttonsTexts=("Cancel", "Done")
    )
    if msg.cancel:
        return False
    ...
    mainWin.log("[Data Re-Struct] Asking in which folder to save the images files...")
    ...
    if selectHowWin.selectedItemIdx == 0:
        ...
        txt = html_utils.paragraph(
            "Do you want to <b>copy or move</b> the files to the Position folders?"
        )
        msg.question(
            mainWin,
            "Copy or move files?",
...
def checkFileFormat(folderPath, mainWin):
    ...
    if not files:
        ...
        msg.critical(mainWin, "Multiple extensions detected", txt)
        return []
    all_ext = [
    ...
    if not is_ext_unique:
        ...
        _, proceedWithMostCommon = msg.warning(
            mainWin, "Multiple extensions detected", txt, buttonsTexts=("Cancel", "Yes")
        )
...
def _run_multi_files_timepoints(mainWin, validFilenames, rootFolderPath, dstFolderPath):
    ...
    matching_frame_name_pattern, frameNumber = get_frame_num_and_pattern(sampleFilename)
    mainWin.thread = QThread()
    mainWin.restructWorker = workers.RestructMultiTimepointsWorker(
        win.allChannels,
        matching_frame_name_pattern,
        win.basename,
        validFilenames,
        rootFolderPath,
        dstFolderPath,
    ...
    mainWin.restructWorker.signals.finished.connect(mainWin.restructWorker.deleteLater)
