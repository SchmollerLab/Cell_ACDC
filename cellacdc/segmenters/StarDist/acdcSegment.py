...
class Model:
    def __init__(
        self, model_name: AvailableModels = "2D_versatile_fluo", load_stardist_3D=False
    ):
        """_summary_
        ...
        if model_name in stardist_default_models:
            ...
            self.model = model_class(None, name=model_name, basedir=model_path)
        self.load_stardist_3D = load_stardist_3D

    def segment(self, image, prob_thresh=0.0, nms_thresh=0.0, segment_3D_volume=False):
        ...
        if not segment_3D_volume and image.ndim == 3:
            ...
            for i, _img in enumerate(image):
                lab, _ = self.model.predict_instances(
                    normalize(_img), prob_thresh=prob_thresh, nms_thresh=nms_thresh
                )
                labels[i] = lab
