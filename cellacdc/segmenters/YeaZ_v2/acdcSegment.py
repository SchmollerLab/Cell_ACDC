...
class Model:
    ...
    def _segment_img_3D(self, image, thresh_val=0.0, min_distance=10):
        ...
        for i, pred in enumerate(prediction):
            ...
            lab = yeaz_segment.segment(thresh, pred, min_distance=min_distance)
    ...
    def _segment_img_2D(self, image, thresh_val=0.0, min_distance=10, warn=True):
        ...
        lab = yeaz_segment.segment(thresholded, prediction, min_distance=min_distance)
        ...
        if image.ndim == 3:
            ...
            for z, img in enumerate(image):
                lab = self._segment_img_2D(
                    img, thresh_val=thresh_val, min_distance=min_distance, warn=z == 0
                )
                labels[z] = lab
        else:
