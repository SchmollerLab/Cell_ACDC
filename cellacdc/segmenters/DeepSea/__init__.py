...
def _init_model(checkpoint_filename, DeepSeaClass, gpu=False):
    ...
    checkpoint_path = os.path.join(deepsea_segmenters_path, checkpoint_filename)
    checkpoint = torch.load(checkpoint_path, map_location=torch_device)

    model = DeepSeaClass(n_channels=1, n_classes=2, bilinear=True)
