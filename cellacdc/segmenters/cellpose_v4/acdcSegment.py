...
class Model(CellposeBaseModel):
    ...
    def segment(
        ...
        labs = self.eval_loop(image, segment_3D_volume, **eval_kwargs)
    ...
    def segment3DT(self, video_data, signals=None, **kwargs):
        ...
        image = video_data[0]
        eval_kwargs, self.isZstack = self.get_eval_kwargs(image, **kwargs)

        eval_kwargs = self._get_eval_kwargs_v4(**kwargs, prev_kwargs=eval_kwargs)

        labels = self.segment3DT_eval(video_data, eval_kwargs, **kwargs)
