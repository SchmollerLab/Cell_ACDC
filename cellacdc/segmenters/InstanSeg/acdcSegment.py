...
class ChannelOrder:
    values = ("First channel", "Second channel")
...
class Model:
    ...
    def segment(
        ...
        if is_zstack:
            ...
            lab = self._segment_2D_img(image_in, PhysicalSizeX, masks_index=masks_index)

        return lab

    def _segment_2D_img(self, image, PhysicalSizeX, masks_index=0):
        labeled_output, image_tensor = self.model.eval_small_image(image, PhysicalSizeX)
