...
class Model:
    ...
    def get_zStack_rgb(self, image):
        ...
        self.is_rgb = (
            (self.img_shape[-1] == 3 or self.img_shape[-1] == 4)
            if not self.is_rgb
            else self.is_rgb
        )
        ...
        self.isZstack = remaining_dims == 3
    ...
    def get_eval_kwargs(
        ...
        **kwargs,
        ...
        if stitch_threshold > 0:
            if not self.printed_model_params:
                print("Using stiching mode instead of trying to segment 3D volume.")
            do_3D = False

        if isZstack and flow_threshold > 0:
            if not self.printed_model_params:
                print(
                    "Flow threshold is not used for 3D segmentation. Setting it to 0.0."
                )
            flow_threshold = 0.0
    ...
    def eval_loop(self, images, segment_3D_volume, init_imgs=True, **eval_kwargs):
    ...
    def segment3DT_eval(self, images, eval_kwargs, init_imgs=True, **kwargs):
        if not kwargs["segment_3D_volume"] and self.isZstack:
            ...
            self.z_axis = (
                z_axis - 2 if z_axis is not None else None
            )  # video doesnt count as dim. iterate over time
            ...
            self.z_axis = (
                z_axis - 1 if z_axis is not None else None
            )  # video doesnt count as dim
...
def check_directml_gpu_gpu(model_name, directml_gpu, gpu, ask_install=True):
    ...
    if directml_gpu:
        from cellacdc.segmenters._cellpose_base._directML import init_directML
...
def setup_gpu_direct_ml(self, directml_gpu, gpu, device):
    ...
    if directml_gpu:
        from cellacdc.segmenters._cellpose_base._directML import setup_directML
    ...
    if gpu:  # sometimes gpu is not properly set up ^^
        from cellacdc.segmenters._cellpose_base._directML import setup_custom_device
...
def _get_normalize_params(
    ...
    normalize_kwargs = {}
    do_rescale = (
        rescale_intensity_low_val_perc != 0 or rescale_intensity_high_val_perc != 100.0
    )
    if not do_rescale:
