...
class Model(CellposeBaseModel):
    ...
    def __init__(
        ...
        if denoise_before_segmentation and denoise_model:
            ...
            if denoise_model_type != model_type:
                printl(
                    f"[WARNING] denoise model type {denoise_model_type} does not match "
                )
    ...
    def segment(  # 2D, 2D x stacks. 2D over time is in segment3DT, 4D is not supported
        ...
        eval_kwargs["normalize"] = (
            norm_kwargs if self.denoiseModel is None else True
        )  # if denoise model was used, just normalise the image with default parameters
    ...
    def segment3DT(
        self, video_data, signals=None, init_imgs=True, **kwargs
    ):  # just 2D over time
        self.timelapse = True
        ...
        if self.denoiseModel is not None:
            if init_imgs:
                if not kwargs["segment_3D_volume"] and self.isZstack:
                    video_data, z_axis, channel_axis = _initialize_image(
                        video_data,
                        self.is_rgb,
                        iter_axis_time=0,
                        iter_axis_zstack=1,
