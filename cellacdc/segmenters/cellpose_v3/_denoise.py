...
from cellacdc.segmenters._cellpose_base.acdcSegment import (
    ...
    DealWithSecondChannelOptions,
    check_deal_with_second_channel,
)
import torch
...
class CellposeDenoiseModel(DenoiseModel):
    def __init__(
        ...
        self.is_rgb = is_rgb
        (
            self.denoise_second_channel_separately,
            self.denoise_second_channel_together,
            self.ignore_second_channel,
        ...
        super().__init__(
            gpu=gpu,
            pretrained_model=denoise_model_path,
            diam_mean=diam_mean,
            chan2=self.denoise_second_channel_together,
            nchan=denoise_nchan,
            device=device,
            model_type=denoise_model,
        )

        setup_gpu_direct_ml(self, directml_gpu, gpu, device)
    ...
    def run(
        ...
        if self.denoise_second_channel_together:
            ...
            self.denoise_second_channel_separately
            or self.ignore_second_channel
            or not self.is_rgb
        ):
            eval_kwargs["channels"] = self.cellpose_greyscale_channel
        else:
            raise ValueError(f"Invalid channels configuration for denoising!")

        iter_axis_zstack = None if not self.denoise_slices_separately else 0
        iter_axis_zstack = (
            iter_axis_zstack + 1
            if (timelapse and iter_axis_zstack is not None)
            else iter_axis_zstack
        )
        if init_image:
        ...
        if timelapse:
            pbartime = tqdm(total=len(image), ncols=100, desc="Denoising time-lapse: ")
        else:
            pbartime = tqdm(total=1, ncols=100, desc="Denoising image: ")

        if timelapse:
            for t, img in enumerate(image):
                denoised_img = self._eval_image(img, eval_kwargs, denoised_img, t=t)
                pbartime.update(1)
        else:
            denoised_img = self._eval_image(image, eval_kwargs, denoised_img)
            pbartime.update(1)
    ...
    def _eval_image(self, image, eval_kwargs, entire_denoised_img, t=None):
        ...
        if self.denoise_slices_separately:
            if self.denoise_second_channel_separately:  # dont need to move channel axis in output since I iterate over it and put it back correctly
                for z, c in tqdm(
                    itertools.product(
                        range(len(image)), self.first_second_channel
                    ),  # only denoise channels which are requested
                    desc=f"Denoising z-slicesand channels",
                    ncols=100,
                ):
                    img = image[z][c]
                ...
                for z, img_z in tqdm(
                    enumerate(image), desc=f"Denoising z-slices: ", ncols=100
                ):
                ...
                denoised_img[:, self.first_second_channel[1]] = image[
                    :, self.first_second_channel[1]
                ]
            ...
            if self.denoise_second_channel_separately:  # dont need to move channel axis in output since I iterate over it and put it back correctly
                ...
                for c in tqdm(
                    self.first_second_channel, desc=f"Denoising channels: ", ncols=100
                ):
                    img = self._acdc_eval(image[c], eval_kwargs)
                    denoised_img[c] = np.squeeze(
                        img
                    )  # remove channel axis if it was added
                if self.isZstack:
                ...
                img = np.squeeze(img)  # remove channel axis if it was added
                denoised_img[self.first_second_channel[0]] = (
                    img  # remove channel axis if it was added
                )
                ...
                if self.isZstack:
                    denoised_img = np.moveaxis(
                        denoised_img, 0, 1
                    )  # move channel axis to the back
            else:
                denoised_img = self._acdc_eval(
                    image, eval_kwargs
                )  # pass entire iamge, with or without channels. Channels param set before
                if self.is_rgb:
                    if self.isZstack:
                        denoised_img = np.moveaxis(
                            denoised_img, -1, 1
                        )  # move channel axis to the front after z
                    else:
                        denoised_img = np.moveaxis(
                            denoised_img, -1, 0
                        )  # move channel axis to the front
                else:
                    denoised_img = np.squeeze(
                        denoised_img
                    )  # remove channel axis if it was added
    ...
    def _add_rgb_channels(self, denoised_img: np.ndarray, isZstack=None):
        ...
        if isZstack:
            ...
            if channels < 3:
                shape_to_concat = (
                    denoised_image_shape[0],
                    3 - channels,
                    denoised_image_shape[2],
                    denoised_image_shape[3],
                )
