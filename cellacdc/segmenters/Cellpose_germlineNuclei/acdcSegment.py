...
default_model_path = os.path.join(
    user_profile_path, "acdc-Cellpose_germlineNuclei", "cellpose_germlineNuclei_2023"
)


class Model:
    def __init__(self, model_path: os.PathLike = default_model_path, gpu=False):
    ...
    def segment(
        ...
        if not isZstack:
            ...
            img_scaled = np.zeros(
                (
                    image.shape[0],
                    round(image.shape[1] * pxScale),
                    round(image.shape[2] * pxScale),
                )
            )
            ...
            if pxScale > 1:
                for i in range(image.shape[0]):
                    img_scaled[i, :, :] = scipy.ndimage.zoom(
                        image[i, :, :], pxScale, order=3
                    )
                    img_blur[i, :, :] = scipy.ndimage.gaussian_filter(
                        img_scaled[i, :, :], blurfactor
                    )
            ...
            labels_scaled, flows_blur, styles_blur = self.model.eval(
                img_blur.astype(np.uint16),
                diameter=30,
                channels=channels,
                do_3D=True,
            ...
            for i in range(image.shape[0]):
                labels[i, :, :] = scipy.ndimage.zoom(
                    labels_scaled[i, :, :],
                    (
                        image.shape[1] / labels_scaled.shape[1],
                        image.shape[2] / labels_scaled.shape[2],
                    ),
                    order=0,
                )
            ...
            if clean_borders:
                idx = np.unique(
                    np.concatenate(
                        [
                            np.unique(labels[-1, :, :][labels[-1, :, :] > 0]),
                            np.unique(labels[0, :, :][labels[0, :, :] > 0]),
                            np.unique(labels[:, 0:2, :][labels[:, 0:2, :] > 0]),
                            np.unique(labels[:, -3:-1, :][labels[:, -3:-1, :] > 0]),
                            np.unique(labels[:, :, 0:2][labels[:, :, 0:2] > 0]),
                            np.unique(labels[:, :, -3:-1][labels[:, :, -3:-1] > 0]),
                        ]
                    )
