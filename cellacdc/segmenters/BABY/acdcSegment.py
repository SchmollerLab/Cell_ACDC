...
class Model:
    ...
    def segment(
        self, image, refine_outlines=True, swap_YX_axes_to_XY=True, PhysicalSizeX=1.0
    ):
        Y, X = image.shape[-2:]
        ...
        for result in result_generator:
            masks = result["masks"]
            areas_mapper = {
                m: (np.count_nonzero(mask), mask) for m, mask in enumerate(masks)
            }
            areas_mapper = dict(
                sorted(areas_mapper.items(), key=lambda item: item[1][0], reverse=True)
            )
