...
class Model:
    def __init__(
        self, model_type: AvailableModels = "bact_phase_omni", net_avg=False, gpu=False
    ):
        if model_type not in OMNI_MODELS:
            err_msg = (
                f'"{model_type}" not available. Available models are {OMNI_MODELS}'
            )
            raise NameError(err_msg)
        self.model = models.Cellpose(gpu=gpu, net_avg=net_avg, model_type=model_type)
