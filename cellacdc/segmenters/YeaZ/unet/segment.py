...
def cell_merge(wsh, pred, q=0.75):
    ...
    for obj1 in range(wsh.max()):
        ...
        for obj2 in range(obj1 + 1, wsh.max()):
            ...
            # is not yet deleted
            if (
                do_box_overlap(obj_coords[obj1, :], obj_coords[obj2, :])
                and np.sum(dil2) > 0
            ):
                border = dil1 * dil2
                ...
                # merge cells
                if top_border_height / top_border_area > 0.99:
...
def do_box_overlap(coord1, coord2):
    ...
    return (
        coord1[0] - 2 < coord2[0]
        and coord1[1] + 2 > coord2[0]
        or coord2[0] - 2 < coord1[0]
        and coord2[1] + 2 > coord1[0]
    ) and (
        coord1[2] - 2 < coord2[2]
        and coord1[3] + 2 > coord2[2]
        or coord2[2] - 2 < coord1[2]
        and coord2[3] + 2 > coord1[2]
    )
...
def get_bounding_box(im):
    ...
    return np.array(
        [np.min(coords[0]), np.max(coords[0]), np.min(coords[1]), np.max(coords[1])]
    )
