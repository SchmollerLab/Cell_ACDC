...
def warnRestartCellACDCcolorModeToggled(scheme, app_name="Cell-ACDC", parent=None):
    ...
    msg = widgets.myMessageBox(wrapText=False)
    txt = f"In order for the change to take effect, <b>please restart {app_name}</b>"
    if scheme == "dark":
...
def warn_cca_integrity(txt, category, qparent, go_to_frame_callback=None):
    ...
    stopSpecificMessageCheckbox = QCheckBox("Stop warning with this specific message")
    stopCategoryCheckbox = QCheckBox(f'Stop warning about "{category}"')
    disableAllWarningsCheckbox = QCheckBox("Disable all warnings")
    ...
    if go_to_frame_callback is not None and txt.find("At frame n.") != -1:
        ...
        goToFrameButton.clicked.connect(partial(go_to_frame_callback, int(frame_n)))

    msg.warning(qparent, "Annotations integrity warning", msg_text, widgets=checkboxes)
...
def warn_installing_different_cellpose_version(requested_version, installed_version):
    ...
    """)
    msg = widgets.myMessageBox(wrapText=False)
    msg.warning(None, "Cellpose already installed", txt, buttonsTexts=("Cancel", "Ok"))
    return msg.cancel
...
def warnPromptSegmentModelNotInit(qparent=None):
    ...
    msg.warning(
        qparent,
        "Promptable model not initialized",
        txt,
    )
    return msg.cancel
...
def log_pytorch_not_installed():
    print(error_below)
    print(
        f"PyTorch is not installed. See here how to install it {urls.install_pytorch}"
    )
    print(error_close)
...
def warnExportToVideo(qparent=None):
    ...
    msg = widgets.myMessageBox(wrapText=False)
    msg.warning(
        qparent, "Export to video is starting", txt, buttonsTexts=("Cancel", "Ok")
    )
    return msg.cancel
...
def warnDivisionAnnotationCannotBeUndone(ID, relID, issue_frame_i, qparent=None):
    ...
    """)
    msg = widgets.myMessageBox(wrapText=False)
    msg.warning(qparent, "Division annotation cannot be undone", txt)
    return msg.cancel
...
def warnMissingCca(missing_cca_items, qparent=None):
    ...
    for cca_df, posData, frame_i in missing_cca_items:
        txt = f"<b>{posData.pos_foldername}</b>:<br><br>"
        indent = "&nbsp;&nbsp;"
        if frame_i is not None:
            txt = f"{txt}  - Frame n. {frame_i + 1}</b>:<br>"
            indent = "&nbsp;&nbsp;&nbsp;&nbsp;"
        missing_IDs = cca_df[cca_df.isnull().any(axis=1)].index.to_list()
        for missing_ID in missing_IDs:
            txt = f"{txt}{indent}* ID: {missing_ID}<br>"
    ...
    _, ignoreButton = msg.warning(
        qparent,
        "Missing cell cycle annotations",
        mainText,
        detailsText,
