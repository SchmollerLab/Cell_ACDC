...
class DataLoadingView:
    ...
    LEGACY_METHODS = (
        ...
        'zSliceAbsent',
        '_workerDebug',
        'warnMemoryNotSufficient',
        'checkMemoryRequirements',
        'loadPosTriggered',
        'startAutomaticLoadingPos',
        'stopAutomaticLoadingPos',
        'loadNonAlignedFluoChannel',
        'load_fluo_data',
        'initFluoData',
        'getPathFromChName',
        'loadFluo_cb',
        'criticalFluoChannelNotFound',
        'loadSelectedData',
        'startLoadDataWorker',
        'askRecoverNotSavedData',
        'showInfoAutosave',
        'askMismatchSegmDataShape',
        ...
        'getMostRecentPath',
        '_openFolder',
        '_loadFromExperimentFolder',
        'criticalInvalidPosFolder',
        'openFile',
        'askUserChannelName',
        'warnUserCreationImagesFolder',
        '_openFile',
        'criticalNoTifFound',
        '_createEmptyData',
        'newFile',
        'helpNewFile',
        'criticalImgPathNotFound',
        'openRecentFile',
    )
    ...
    def warnMemoryNotSufficient(self, total_ram, available_ram, required_ram):
        ...
        cancelButton, continueButton = msg.warning(
            self.host, 'Memory not sufficient', txt,
    ...
    def startAutomaticLoadingPos(self):
        self.AutoPilot = autopilot.AutoPilot(self.host)
    ...
    def loadNonAlignedFluoChannel(self, fluo_path):
        ...
        if posData.filename.find('aligned') != -1:
            ...
            msg = widgets.myMessageBox()
            msg.critical(
                self.host, 'Aligned fluo channel not found!',
    ...
    def load_fluo_data(self, fluo_path, isGuiThread=True):
        ...
        if ext == '.npy' or ext == '.npz':
            ...
            msg = widgets.myMessageBox()
            msg.critical(self.host, 'File not supported', txt)
            return None, None
    ...
    def initFluoData(self):
        ...
        no, yes = msg.question(
            self.host, 'Load fluorescence images?', html_utils.paragraph(txt),
            buttonsTexts=('No', 'Yes')
        )
    ...
    def loadFluo_cb(self, checked=True, fluo_channels=None):
        if fluo_channels is None:
            ...
            if not ch_names:
                ...
                msg.information(self.host, 'All channels are loaded', txt)
                return False
    ...
    def criticalFluoChannelNotFound(self, fluo_ch, posData):
        ...
        okButton = msg.warning(
            self.host, title, txt, buttonsTexts=('Ok')
        )
    ...
    def loadSelectedData(self, user_ch_file_paths, user_ch_name):
        ...
        posData.getBasenameAndChNames(qparent=self.host)
        posData.buildPaths()
        ...
        if self.isNewFile or not existingSegmEndNames:
            ...
            if len(existingSegmEndNames) > 0:
                win = apps.SelectSegmFileDialog(
                    existingSegmEndNames, self.exp_path, parent=self.host,
        ...
        self.progressWin = apps.QDialogWorkerProgress(
            title='Loading data...', parent=self.host,
    ...
    def startLoadDataWorker(
        ...
        self.loadDataWorker = workers.loadDataWorker(
            self.host, user_ch_file_paths, user_ch_name, firstPosData
        )
    ...
    def askMismatchSegmDataShape(self, posData):
        ...
        _, skipPosButton = msg.warning(
            self.host, title, text,
            buttonsTexts=('Cancel', 'Skip this Position')
        )
    ...
    def workerPermissionError(self, txt, waitCond):
        msg = widgets.myMessageBox(parent=self.host)
    ...
    def loadingDataCompleted(self):
        ...
        self.preprocessing_view.setupPreprocessing()
        ...
        self.measurements_view.init_metrics()
        self.initFluoData()
        ...
        self.mode_controls_view.setFramesSnapshotMode()
        if self.isSnapshot:
        ...
        self.custom_annotations_view.readSavedCustomAnnot()
        self.custom_annotations_view.addCustomAnnotButtonAllLoadedPos()
        ...
        self.measurements_view.set_metrics_func()
        ...
        self.image_controls_view.setFocusMain()
        ...
        if ch_name_selector.is_first_call and user_ch_name is None:
            ...
            if len(ch_names) > 1:
                ...
                ch_name_selector.QtPrompt(
                    self.host, ch_names, CbLabel=CbLabel
                )
    ...
    def _loadFromExperimentFolder(self, exp_path):
        ...
        if len(values) > 1:
            select_folder.QtPrompt(self.host, values, allow_cancel=False)
    ...
    def criticalInvalidPosFolder(self, exp_path):
        ...
        msg.critical(
            self.host, 'Incompatible folder', txt
        )
    ...
    def warnUserCreationImagesFolder(self, images_path, ext):
        ...
        if ext == '.tif' or ext == '.npz':
            ...
            _, copyButton, moveButton = msg.information(
                self.host, 'Creating Images folder', txt,
    ...
    def _openFile(self, file_path=None):
        ...
        if file_path is None:
            ...
            file_path = QFileDialog.getOpenFileName(
                self.host, 'Select image file', self.MostRecentPath,
        ...
        if context.requires_images_folder:
            proceed, do_copy = self.warnUserCreationImagesFolder(
                context.exp_path, context.extension
            )
            if not proceed:
        ...
        if target.direct_copy_supported:
            if not os.path.exists(target.new_filepath):
                ...
                if do_copy:
                    shutil.copy2(file_path, target.new_filepath)
                else:
                    shutil.move(file_path, target.new_filepath)
            self._openFolder(
                exp_path=context.exp_path, imageFilePath=target.new_filepath
            )
        else:
            ...
            data.img_data = preparation.image

            myutils.to_tiff(target.tif_path, data.img_data)
            self._openFolder(
                exp_path=context.exp_path, imageFilePath=target.tif_path
            )
    ...
    def criticalNoTifFound(self, images_path):
        ...
        msg.critical(self.host, err_title, err_msg)
    ...
    def _createEmptyData(self):
        ...
        os.makedirs(plan.images_path, exist_ok=True)

        empty_img = np.zeros((256,256), dtype=np.uint8)
        empty_img[0,0] = 255
        skimage.io.imsave(plan.tif_filepath, empty_img)

        df_metadata = pd.DataFrame({
            'Description': ['basename'],
            'values': [plan.basename]
        })
        df_metadata.to_csv(plan.metadata_filepath, index=False)

        self.isNewFile = True
        self._openFolder(exp_path=plan.images_path)

    def newFile(self):
        self.newSegmEndName = ''
        self.isNewFile = True
        msg = widgets.myMessageBox(parent=self.host, showCentered=False)
    ...
    def helpNewFile(self):
        ...
        """)
        msg.information(
            self.host, 'Help on Position folders', txt
        )
    ...
    def criticalImgPathNotFound(self, images_path):
        ...
        okButton = msg.critical(
            self.host, 'No valid files found!', err_msg, buttonsTexts=('Ok',)
        )
