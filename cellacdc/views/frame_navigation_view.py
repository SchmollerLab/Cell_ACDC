...
class FrameNavigationView:
    """Qt-facing adapter for frame and position navigation workflows."""

    LEGACY_METHODS = (
        'goToZsliceSearchedID',
        'isNavigateActionOnNextFrame',
        'nextFrameImage',
        'rightImageFramesScrollbarValueChanged',
        'nextActionTriggered',
        'prevActionTriggered',
        'resetNavigateScrollbar',
        'next_cb',
        'prev_cb',
        'updateScrollbars',
        'updateFramePosLabel',
        'updateItemsMousePos',
        'setNavigateScrollBarMaximum',
        'setFrameNavigationDisabled',
        'navigateSpinboxValueChanged',
        'navigateSpinboxEditingFinished',
        'PosScrollBarAction',
        'PosScrollBarMoved',
        'PosScrollBarReleased',
        'resetNavigateFramesScrollbar',
        'framesScrollBarActionTriggered',
        'framesScrollBarMoved',
        'framesScrollBarReleased',
        'next_pos',
        'updatePos',
        'prev_pos',
        'updateViewerWindow',
        'warnLostObjects',
        'warnReinitLastSegmFrame',
        'extendSegmDataIfNeeded',
        'reInitLastSegmFrame',
        'resetAcceptedLostIDs',
        'askInitCcaFirstFrame',
        'askInitLinTreeFirstFrame',
        'checkIfFutureFrameManualAnnotPastFrames',
        'next_frame',
        'apply_tools_on_new_frame',
        'manualAnnotRestoreLastTrackedFrame',
        'prev_frame',
        'connectScrollbars',
        'zSliceScrollBarActionTriggered',
        'zSliceScrollBarReleased',
        'setSwitchViewedPlaneDisabled',
        '_setViewRangeSwitchPlane',
        'setViewRangeSwitchPlane',
        'switchViewedPlane',
        'onZsliceSpinboxValueChange',
        'update_z_slice',
        'updateOverlayZslice',
        'updateOverlayZproj',
        'updateZproj',
    ...
    def isNavigateActionOnNextFrame(self):
        ...
        return self.view_model.should_show_next_frame_image(
            ...
            action_checked=self.labelsGrad.showNextFrameAction.isChecked(),
        )
    ...
    def setNavigateScrollBarMaximum(self):
        ...
        limit = self.view_model.navigation_limit(
            ...
            lineage_tree_frames=lineage_frames,
        )
        if limit is None:
            return

        self.navigateScrollBar.setMaximum(limit.maximum)
        self.navSpinBox.setMaximum(limit.maximum)
        if limit.last_checked_frame_i is not None:
            self.updateLastCheckedFrameWidgets(limit.last_checked_frame_i)
        if limit.status_text is not None:
            self.lastTrackedFrameLabel.setText(limit.status_text)
    ...
    def framesScrollBarMoved(self, frame_n):
        ...
        self.display_decorations_view.update_timestamp_frame()
    ...
    def updatePos(self):
        ...
        self.preprocessing_view.updatePreprocessPreview()
        ...
        self._sync_session(self.pos_i)
        self._sync_session_frame_i(self.pos_i)
    ...
    def warnLostObjects(self, do_warn=True):
        ...
        should_warn = self.view_model.should_warn_lost_objects(
            ...
            mode=mode,
            lost_ids=posData.lost_IDs,
            already_accepted=already_accepted_lost,
        )
        if not should_warn:
            return True
    ...
    def warnReinitLastSegmFrame(self):
        ...
        """)
        msg.warning(
            self.host, 'WARNING: Potential loss of data', txt,
    ...
    def askInitCcaFirstFrame(self):
        ...
        editCcaWidget = apps.editCcaTableWidget(
            posData.cca_df, posData.SizeT, parent=self.host,
    ...
    def next_frame(self, warn=True):
        ...
        self.label_transform_tools_view.reset_expand_label()
        self.updateAllImages()
        self.updateHighlightedAxis()
        self.updateViewerWindow()
    ...
    def apply_tools_on_new_frame(self):
        ...
        should_apply = self.view_model.should_apply_new_frame_tools(
            ...
            last_tracked_i=posData.last_tracked_i,
            frame_i=posData.frame_i,
            last_frame_ran=self.lastFrameRanOnFirstVisitTools,
        )
        if not should_apply:
            return
    ...
    def connectScrollbars(self):
        ...
        if self.data[0].SizeZ > 1:
            ...
            self.zProjComboBox.activated.connect(
    ...
    def switchViewedPlane(self, previousPlane, currentPlane):
        ...
        if depthAxes != 'z' and self.isSnapshot:
            ...
            self.mode_controls_view.disableEditingViewPlaneNotXY()
        elif self.isSnapshot:
            # Re-enable editing in snapshot mode when the plane is xy
            self.mode_controls_view.setEnabledSnapshotMode()
    ...
    def update_z_slice(self, z):
        ...
        if self.switchPlaneCombobox.depthAxes() == 'z':
            idx = self.view_model.z_slice_frame_indices(
                ...
                frame_i=posData.frame_i,
                size_t=posData.SizeT,
                locked=self.zProjLockViewButton.isChecked(),
            )
