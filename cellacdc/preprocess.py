...
def remove_hot_pixels(
    image, logger_func=print, progress=True, apply_to_all_zslices=True
):
...
def ridge_filter(image, sigmas: _types.Vector = (1.0, 2.0), apply_to_all_zslices=True):
...
def spot_detector_filter(
    ...
    blurred1 = gaussian_filter(image, sigma1, use_gpu=use_gpu, logger_func=logger_func)

    sigma2 = SQRT_2 * sigma1
    blurred2 = gaussian_filter(image, sigma2, use_gpu=use_gpu, logger_func=logger_func)
...
def enhance_speckles(img, radius=15, apply_to_all_zslices=False):
...
def dummy_filter(
    image: np.ndarray, apply_to_all_zslices=False, apply_to_all_frames=False
):
    printl(image.shape)
    return image
...
class VolumeImageData:
    ...
    def __setitem__(self, z_slice: int, image: np.ndarray):
    ...
    def __getitem__(self, z_slice: Union[int, Tuple[Union[int, slice]], None]):
...
def basicpy_background_correction(
    ...
    basic.fit(images)
    images = basic.transform(images, timelapse=timelapse)
