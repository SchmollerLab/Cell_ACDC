...
def channel_indipendent_metric(
    ...
    ch1_amount_key = [
        key for key in metrics_values if key.startswith(f"{channel_1}_amount_autoBkgr")
    ][0]

    ch1_amount = metrics_values[ch1_amount_key]

    ch2_amount_key = [
        key for key in metrics_values if key.startswith(f"{channel_2}_amount_autoBkgr")
    ][0]
