...
def find_available_channels(filenames, first_pos_dir):
    ...
    ch_names, warn = ch_name_selector.get_available_channels(filenames, first_pos_dir)
        ...
        for pos_idx, pos_dir in enumerate(image_folders[file_idx]):
            ...
            try:
                *channel_data, seg_mask, cc_data, metadata, cc_props, acdc_df_path = (
                ...
                print(
                    f"File {file}, position {positions[file_idx][pos_idx]} skipped due to missing segmentation mask/CC annotations."
                )
                continue
            print(f"Number of cells in position: {len(cc_data.Cell_ID.unique())}")
            print(
                f"Number of annotated frames in position: {cc_data.frame_i.max() + 1}"
            )
            ...
            if cc_props is not None and not force_recalculation:
                ...
                overall_df = pd.concat(
                    [overall_df, cc_props], ignore_index=True
                ).reset_index(drop=True)
            else:
                print(f"Calculate regionprops on each frame based on Segmentation...")
                rp_df = _calculate_rp_df(
                    seg_mask,
                    is_timelapse_data,
                    is_zstack_data,
                    metadata,
                    max_frame=cc_data.frame_i.max() + 1,
                )
                ...
                temp_df = cc_data.merge(
                    rp_df, on=["frame_i", "Cell_ID"], how="left", suffixes=("_gui", "")
                )
                ...
                if is_timelapse_data:
                    for channel in channels:
                        temp_df[f"{channel}_corrected_amount"] = (
                            temp_df[f"{channel}_corrected_mean"] * temp_df["area"]
                        )
                        try:
                            ...
                            print(
                                f"Volume is missing in acdc output, NaNs inserted in concentration columns of channel {channel}"
                            )
                ...
                overall_df = pd.concat(
                    [overall_df, temp_df], ignore_index=True
                ).reset_index(drop=True)
...
def calculate_relatives_data(overall_df, channels):
    ...
    for ch in channels:
        try:
            ...
            overall_df[f"{ch}_combined_raw_sum_mother_bud"] = overall_df.apply(
                lambda x: (
                    x.loc[f"{ch}_raw_sum"] + x.loc[f"{ch}_raw_sum_rel"]
                    if x.loc["cell_cycle_stage"] == "S"
    ...
    overall_df["combined_mother_bud_volume"] = overall_df.apply(
        lambda x: (
            x.loc["cell_vol_fl"] + x.loc["cell_vol_fl_rel"]
            if x.loc["cell_cycle_stage"] == "S"
            else x.loc["cell_vol_fl"]
        ),
        axis=1,
    )
    return overall_df
...
def calculate_per_phase_quantities(overall_df, group_cols, channels):
    ...
    phase_grouped = (
        overall_df.sort_values("frame_i")
        .groupby(group_cols)
        .agg(
    ...
    for ch in channels:
        if f"{ch}_corrected_mean" in overall_df.columns:
            ...
            flu_temp.columns = [
                ...
                for col in flu_temp.columns.values
            ]
            ...
            phase_grouped_flu = phase_grouped_flu.merge(
                flu_temp, how="right", on=group_cols, suffixes=("", "")
            )
    ...
    if "max_t" in overall_df.columns:
        complete_phase_indices = np.logical_and(
            temp, phase_grouped.phase_end < phase_grouped.max_t
        )
    else:
    ...
    phase_grouped = phase_grouped.merge(
        phase_grouped_flu, how="left", on=group_cols, suffixes=("", "")
    )
...
def load_acdc_output_only(file_names, image_folders, positions, segm_endnames):
    """
    ...
    for file_idx, file in enumerate(file_names):
        ...
        for pos_idx, pos_dir in enumerate(image_folders[file_idx]):
            try:
                ...
                cc_stage_path = glob.glob(os.path.join(f"{pos_dir}", "*cc_stage.csv"))[
                    0
                ]
...
def _load_channels_data(file_dir, channel_names, no_of_aligned_files):
    ...
    if no_of_aligned_files > 0:
        for channel in channel_names:
            try:
                ch_aligned_path = glob.glob(
                    os.path.join(f"{file_dir}", f"*{channel}_aligned.npz")
                )[0]
                channel_files.append(np.load(ch_aligned_path)["arr_0"])
            except IndexError:
                try:
                    ch_aligned_path = glob.glob(
                        os.path.join(f"{file_dir}", f"*{channel}_aligned.npy")
                    )[0]
                    channel_files.append(np.load(ch_aligned_path))
                except IndexError:
                    print(f"Could not find an aligned file for channel {channel}")
                    print(
                        f"Resulting data will not contain fluorescent data for this channel"
                    )
                    channel_files.append(None)
    else:
        for channel in channel_names:
            try:
                ch_not_aligned_path = glob.glob(
                    os.path.join(f"{file_dir}", f"*{channel}.tif")
                )[0]
...
def _load_files(file_dir, channels, segm_endname, load_channels_data=True):
    ...
    seg_mask_available = (
        len(glob.glob(os.path.join(f"{file_dir}", f"*_{segm_endname}.npz"))) > 0
    )
    acdc_output_available = (
        len(glob.glob(os.path.join(f"{file_dir}", f"*{acdc_output_endname}.csv")))
        + len(glob.glob(os.path.join(f"{file_dir}", "*cc_stage*")))
        > 0
    )
...
def _calculate_rp_df(
    ...
    if is_timelapse_data:
        for t, img in enumerate(tqdm(labeled_data)):
            ...
            if img.max() > 0:
                t_rp_df = pd.DataFrame(
                    regionprops_table(img.astype(int), properties=props)
                ...
                if metadata is None:
                    ...
                    vol_vox = [
                        _calc_rot_vol(
                            obj,
                            metadata.loc["PhysicalSizeY"],
                            metadata.loc["PhysicalSizeX"],
                        )[0]
                        for obj in t_rp
                    ]
                    vol_fl = [
        ...
        merged_df["elongation"] = (
            merged_df["major_axis_length"] / merged_df["minor_axis_length"]
        )
        return merged_df
    else:
        rp_df = pd.DataFrame(
            regionprops_table(labeled_data.astype(int), properties=props)
        ).rename(columns=rename_dict)
...
def _calculate_flu_signal(
    seg_mask, channel_data, channels, cc_data, is_timelapse_data, is_zstack_data
):
    """
    ...
    for ch_idx, ch_array in enumerate(channel_data):
        if ch_array is None:
            ...
            ch_medians = [
                np.median(ch_array[t][bg_index[t]]) for t in range(max_frame + 1)
            ]
    ...
    if is_timelapse_data:
        for cell_id in tqdm(cc_data.Cell_ID.unique()):
            ...
            temp_df["frame_i"] = times
            temp_df["Cell_ID"] = cell_id
            index_array = seg_mask[: max_frame + 1] == cell_id
            channel_data_cut = [
                c_arr[: max_frame + 1] if c_arr is not None else None
                for c_arr in channel_data
            ]
...
def calculate_effect_size_cohen(
    data, group1, group2, cat_column="size_category", val_column="Pp38_concentration"
):
    ...
    cohen_s = np.sqrt(((n1 - 1) * s1 + (n2 - 1) * s2) / (n1 + n2 - 2))
    effect_size = (
        np.mean(data_gr1[val_column]) - np.mean(data_gr2[val_column])
    ) / cohen_s
    return effect_size


def calculate_effect_size_glass(
    data, group1, group2, cat_column="size_category", val_column="Pp38_concentration"
):
    assert cat_column in data.columns and val_column in data.columns
    data_gr1 = data[data[cat_column] == group1]
...
def _add_end_of_frame_i_column(acdc_df):
    ...
    grouped_ID_gen_num = will_divice_cca_df_S.groupby(["Cell_ID", "generation_num"])
    ...
    cca_df_with_gen_num_idx = (
        cca_df.reset_index().set_index(["Cell_ID", "generation_num"]).sort_index()
    )
    ...
    for row in end_cc_frame_i_per_cycle.itertuples():
        ...
        idx = (ID, gen_num)
        cca_df_with_gen_num_idx.loc[idx, "end_of_cell_cycle_frame_i"] = end_cc_frame_i
    ...
    acdc_df.loc[cca_df_idx, "end_of_cell_cycle_frame_i"] = cca_df[
...
def _extend_will_divide_to_G1(acdc_df):
    ...
    for ID, gen_num in grouped.groups.keys():
        mask = (acdc_df["Cell_ID"] == ID) & (acdc_df["generation_num"] == gen_num)
...
def add_generation_num_of_relative_ID(
    ...
    acdc_df_by_cellID = acdc_df_by_rel_ID.reset_index().set_index(
        [*prefix_index, "frame_i", "Cell_ID"]
    )
    # Intersection takes care of disappearing relative_IDs
    fixing_idx = acdc_df_to_fix.index.intersection(acdc_df_by_cellID.index)

    acdc_df_to_fix.loc[fixing_idx, "generation_num_relID"] = acdc_df_by_cellID.loc[
        fixing_idx, "generation_num"
    ].values
    index_to_fix = acdc_df_by_frame_i[to_fix_mask].index
    acdc_df_by_frame_i.loc[index_to_fix, "generation_num_relID"] = acdc_df_to_fix[
        "generation_num_relID"
    ].values
...
def get_IDs_gen_num_will_divide_wrong(global_cca_df):
    ...
    ID_gen_num_index = (
        global_cca_df.reset_index().set_index(["Cell_ID", "generation_num"]).index
    )

    # Next generation index
    next_gen_will_divide_cca_df = global_cca_will_divide[
...
def generate_mother_bud_total_df(
    ...
    columns_to_add = [
        col
        for col, operation in column_operation_mapper.items()
    ...
    df_S_tot[columns_to_add] = df_S_tot[columns_to_add] + df_S_bud_relID[columns_to_add]
