...
class NapariArboretumDialog(base.MainThreadSinglePosUtilBase):
    ...
    def launchNapariArboretum(self, posPath):
        ...
        existingEndnames = load.get_endnames(posData.basename, segm_files)

        if len(existingEndnames) > 1:
            win = apps.SelectSegmFileDialog(
                existingEndnames, images_path, parent=self, basename=posData.basename
            )
            win.exec_()
            if win.cancel:
                self.logger.info("napari-arboretum utility aborted.")
        ...
        self.logger.info(
            f"Loading segmentation file ending with {selectedSegmEndName}..."
        )
        ...
        viewer.add_tracks(
            tracks_data, graph=graph, name=acdc_df_endname, properties=properties
        )
