...
INVERTIBLE_COLOR_NAMES = ["label", "S_phase_mother", "G1_phase"]
...
def get_obj_text_cca_annot(obj, acdc_df: pd.DataFrame, is_tree_annot: bool) -> str:
    ID = obj.label
    try:
...
def get_obj_text_annot_opts(
    ...
    is_cca_annot: bool,
    is_new_obj: bool,
    add_num_zslices: bool,
    is_label_tree_annot: bool,
    is_gen_num_tree_annot: bool,
    frame_i: int,
) -> dict:
    if acdc_df is None or not is_cca_annot:
        ...
        text, cca_df_obj = get_obj_text_cca_annot(obj, acdc_df, is_gen_num_tree_annot)
    ...
    def highlightObject(self, obj):
        ...
        for idx, objData in enumerate(self.data):
            if ID == objData["data"]:
                ...
                break
        if objIdx is None:
            objOpts = {"text": str(ID), "bold": True, "color_name": "new_object"}
...
class TextAnnotations:
    ...
    def invertBlackAndWhite(self):
        invertedColors = {name: color[:3] for name, color in self.item.colors().items()}
    ...
    def _createHighResolutionItems(self, allIDs, pxMode=False):
        self.item = TextAnnotationsScatterItem(size=self.fontSize, pxMode=pxMode)
    ...
    def setAnnotations(self, **kwargs):
        ...
        for obj in posData.rp:
            ...
            objOpts = get_obj_text_annot_opts(
                ...
                isCcaAnnot,
                isNewObject,
                isAnnotateNumZslices,
                isLabelTreeAnnotation,
                isGenNumTreeAnnotation,
                posData.frame_i,
            )
