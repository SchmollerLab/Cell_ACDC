...
class LineageInteractionsMixin:
    ...
    def askLineageTreeChanges(self):
        ...
        if (
            self.original_df_lin_tree_i is not None
            ...
            self.get_data()
            self.logger.info(
                "Lineage tree changes were not propagated, going back to original frame."
            )
        ...
        result = self.get_difference_table(
            return_css_separated=True, return_differece=True
        )
        if result is None:
        ...
        propagate_btn, discard_btn, _ = msg.question(
            self,
        ...
        if msg.clickedButton == propagate_btn:
            ...
            posData.allData_li[posData.frame_i]["acdc_df"] = (
                self.original_df_lin_tree.copy()
            )
            self.original_df_lin_tree = None
            self.original_df_lin_tree_i = None
    ...
    def find_mother_action(self, posData, event, ydata, xdata):
        ...
        if (
            acdc_df_frame.loc[ID]["parent_ID_tree"] == new_mother
            and not self.original_mother_skipped
        ...
        acdc_df_frame.at[ID, "parent_ID_tree"] = (
            new_mother  # update mother in the df, no need to propagate or stuff lile this
        )
    ...
    def getDistanceListMissingIDs(self, point, ID):
        ...
        if ID not in self.distanceListMissingIDs.keys():
            prev_rp = posData.allData_li[frame_i - 1]["regionprops"]
            relevant_rp = [obj for obj in prev_rp if obj.label not in posData.IDs]
    ...
    def initLinTree(self, force=False):
        ...
        for i, dict_frame_i in enumerate(posData.allData_li):
            ...
            if (
                df is None
                or "generation_num_tree" not in df.columns
        ...
        if posData.frame_i > last_lin_tree_frame_i:
            ...
            _, yesButton, stayButton = msg.warning(
                ...
                buttonsTexts=(
                    "Cancel",
                    f"Yes, go to frame {last_lin_tree_frame_i + 1}",
    ...
    def initMissingFramesLinTree(
        self, current_frame_i
    ):  # done Need to add partially missing previous frames and loading
        """
        ...
        present_frames = (
            list(self.lineage_tree.frames_for_dfs) if self.lineage_tree else []
        )
        present_frames = [] if not present_frames else present_frames  # deal with None
        missing_frames = [
            frame_i for frame_i in missing_frames if frame_i not in present_frames
        ]
        missing_frames.sort()
    ...
    def missing_frame_indices(
        ...
        missing = [
            frame_i for frame_i in range(current_frame_i + 1) if frame_i not in present
        ]
        missing.sort()
        return missing
    ...
    def propagateLinTreeAction(self, dummy_for_button=None):
        ...
        if posData.frame_i == self.original_df_lin_tree_i:
            self.original_df_lin_tree = posData.allData_li[posData.frame_i][
                "acdc_df"
            ].copy()
    ...
    def should_skip_original_mother(
        ...
        *,
        ...
        return current_parent_id == candidate_parent_id and not original_mother_skipped

    def viewLinTreeInfoAction(self):
        mode = str(self.modeComboBox.currentText())
        if mode != "Normal division: Lineage tree":
            self.logger.info(
                'This action is only available in the "Normal division: Lineage tree" mode.'
            )
            return
        ...
        txt_cells_with_parents = (
            f"<b>Cells with parents</b>:{table_cells_with_parent} <br><br>"
        )
        ...
        txt = css + html_utils.paragraph(
            txt_changes + txt_orphan + txt_lost + txt_cells_with_parents
        )

        msg = widgets.myMessageBox()
        msg.information(self, "lineage tree information", txt)
