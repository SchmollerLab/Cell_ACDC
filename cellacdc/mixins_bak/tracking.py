...
class TrackingMixin:
    ...
    def addManualBackgroundObject(self, x, y):
        ...
        textItem = pg.TextItem(text=str(ID), color="r", anchor=(0.5, 0.5))
    ...
    def initGhostObject(self, ID=None):
        ...
        self.ghostObject.contour = self.getObjContours(self.ghostObject, local=True)
    ...
    def initManualBackgroundObject(self, ID=None):
        ...
        if ID not in posData.IDs:
            self.manualBackgroundObj = None
            self.manualBackgroundToolbar.showWarning(f"The ID {ID} does not exist")
            self.manualBackgroundObjItem.clear()
            return
    ...
    def manualTracking_cb(self, checked):
        ...
        if checked:
            ...
            self.UserEnforced_Tracking_previousStatus = self.UserEnforced_Tracking
    ...
    def realTimeTrackingClicked(self, checked):
        ...
        if isRealTimeTrackingDisabled:
            ...
            yesButton, noButton = msg.question(
                self, "Keep tracking always active?", txt, buttonsTexts=("Yes", "No")
            )
    ...
    def repeatTracking(self):
        ...
        if posData.editID_info:
            ...
            editedIDsInfoItems = [
                f"ID {oldID} --> {newID}" for oldID, newID in editedIDsInfo.items()
            ]
            ...
            """)
            keepManualEditButton = widgets.okPushButton("Keep manually edited IDs")
            ignoreButton = widgets.noPushButton("Ignore manually edited IDs")
            msg.question(
                self,
                "Repeat tracking mode",
                txt,
    ...
    def repeatTrackingVideo(self, checked=False):
        ...
        if start_n - 2 <= last_cca_i and start_n > 1:
            proceed = self.warnRepeatTrackingVideoWithAnnotations(last_cca_i, start_n)
            if not proceed:
                self.logger.info("Tracking aborted.")
                return
    ...
    def trackFrame(
        ...
        if self.trackWithAcdcAction.isChecked():
            ...
            tracked_result = self.tracking_yeaz.correspondence(
                prev_lab, curr_lab, use_modified_yeaz=True, use_scipy=True
            )
        else:
    ...
    def trackFrameCustomTracker(self, prev_lab, currentLab, IDs=None, unique_ID=None):
        ...
        try:
            ...
            if str(err).find("an unexpected keyword argument 'unique_ID'") != -1:
                try:
                    tracked_result = self.realTimeTracker.track_frame(
                        prev_lab, currentLab, IDs=IDs, **self.track_frame_params
                    )
                except TypeError as err:
                ...
                try:
                    ...
                    if (
                        str(err).find("an unexpected keyword argument 'unique_ID'")
                        != -1
                    ):
                        tracked_result = self.realTimeTracker.track_frame(
                            prev_lab, currentLab, **self.track_frame_params
    ...
    def trackManuallyAddedObject(
        self,
        added_IDs: List[int] | int | Set[int],
        isNewID: bool,
        wl_update: bool = True,
        wl_track_og_curr: bool = False,
    ):
        ...
        tracked_lab = self.tracking(
            enforce=True, assign_unique_new_IDs=False, return_lab=True, IDs=added_IDs
        )
        ...
        for added_ID in added_IDs:
            ...
            isTrackedIDalreadyPresentAndNotNew = (
                posData.IDs_idxs.get(trackedID) is not None and added_ID != trackedID
            )
    ...
    def tracking(
        ...
        if return_lab:
            QTimer.singleShot(
                50, partial(self.statusBarLabel.setText, staturBarLabelText)
            )
            return tracked_lab
        ...
        self.setAllTextAnnotations()
        QTimer.singleShot(50, partial(self.statusBarLabel.setText, staturBarLabelText))
    ...
    def warnRepeatTrackingVideoOnVisitedFrames(self, last_tracked_i, start_n):
        ...
        noButton, yesButton = msg.warning(
            self,
            "Repating tracking with annotations!",
            txt,
            buttonsTexts=(
