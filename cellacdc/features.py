...
def get_acdc_df_features(
    ...
    vox_to_fl_3D = posData.PhysicalSizeY * posData.PhysicalSizeX * posData.PhysicalSizeZ
    ...
    for category, metrics_names in grouped_features.items():
        if category == "size":
            ...
            z = posData.zSliceSegmentation(filename, frame_i, errors="ignore")
...
def add_background_metrics_names(
    ...
    backgr_metrics_names = [
        name
        for name in backgr_metrics_names
...
def custom_post_process_segm(
    ...
    channel,
    features_range,
    other_channels_foregr_imgs=None,
    return_delIDs=False,
):
    isSegm3D = lab.ndim == 3
