...
from ._palettes import (
    ...
    _get_highligth_text_background_rgba,
)
...
def rst_to_html(rst_text, parse_urls=False, keep_spacing=False):
    ...
    html_text = html_text.replace("\n", "<br>")
    if keep_spacing:
        html_text = re.sub(r"(\s\s+)", lambda m: "&nbsp;" * len(m.group(0)), html_text)
    return html_text
...
def to_admonition(text, admonition_type="note"):
    ...
    title_row = tag(
        f"<td><b>! {title}</b></td>", tag_info=f'tr bgcolor="{RST_NOTE_DIR_HEX_COLOR}"'
    )
    text_row = tag(
        f"<td>{text}</td>", tag_info=f'tr bgcolor="{RST_NOTE_TXT_HEX_COLOR}"'
    )
    admonition_html = (
