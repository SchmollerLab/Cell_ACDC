...
from cellacdc.segmenters_promptable.utils import build_combined_mask
...
class Model:
    ...
    def _add_object_specific_negative_prompts(self, prompt_id):
        ...
        for prompt, prompt_type in obj_negative_prompts:
            if prompt_type == "point":
                ...
                point_prompt = tuple(prompt[::-1])
                self.model.add_point_interaction(
                    point_prompt, include_interaction=False, run_prediction=True
                )
            else:
                raise ValueError(f"Unsupported prompt type: {prompt_type}")
    ...
    def segment(self, image, treat_other_objects_as_background=True, *args, **kwargs):
        ...
        for prompt_id, value in self.prompt_ids_image_mapper.items():
            ...
            target_tensor = torch.zeros(prompt_image.shape[1:], dtype=torch.uint8)
