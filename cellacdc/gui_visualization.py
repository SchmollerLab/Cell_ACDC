"""Data visualization GUI — load datasets and inspect images/labels."""
...
from . import autopilot, myutils, settings_folderpath
from .gui_bundles import VISUALIZATION_GUI_ROOTS
...
class VisualizationGuiWin(
    ...
    """Viewer-focused GUI: open data and visualize frames."""
    ...
    def __init__(
        ...
        self._appName = "Cell-ACDC Viewer"
    ...
    def run(self, module="acdc_gui_viz", logs_path=None):
        from qtpy.QtGui import QIcon

        QMainWindow.setWindowIcon(self, QIcon(":icon.ico"))
        QMainWindow.setWindowTitle(self, "Cell-ACDC Viewer")
        ...
        if self._version is not None:
            logger.info(f"Initializing viewer GUI v{self._version}")
        else:
            logger.info("Initializing viewer GUI...")
        ...
        self.logger.info(
            f"Viewer GUI ready (bundle: {', '.join(VISUALIZATION_GUI_ROOTS)})."
        )
...
def main():
    ...
    win = VisualizationGuiWin(app, version=version)
    win.run()
    run_event_loop(app)
