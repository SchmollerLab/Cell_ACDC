...
class _Data:
    def __init__(
        self, images_path, intensity_image_path, acdc_df_path, segm_path, basename
    ):
...
class FissionYeastAnnotated(_Data):
    def __init__(self):
        images_path = os.path.join(
            data_path,
            "test_symm_div_acdc_tracker",
            "Images",
        )
        intensity_image_path = os.path.join(images_path, "bknapp_Movie_S1.tif")
        acdc_df_path = os.path.join(images_path, "bknapp_Movie_S1_acdc_output.csv")
        segm_path = os.path.join(images_path, "bknapp_Movie_S1_segm.npz")
        basename = "bknapp_Movie_S1_"
        super().__init__(
            images_path, intensity_image_path, acdc_df_path, segm_path, basename
        )
...
class DeepSeaAnnotation(_Data):
    def __init__(self):
        ...
        intensity_image_path = os.path.join(images_path, "set_22_MESC.tif")
        acdc_df_path = os.path.join(images_path, "set_22_MESC_acdc_output.csv")
        segm_path = os.path.join(images_path, "set_22_MESC_segm.tif")
...
class YeastTimeLapseAnnotated(_Data):
    def __init__(self):
        ...
        segm_path = os.path.join(images_path, "SCGE_5strains_23092021_segm.npz")
...
class pomBseenDualChannelData(_Data):
    def __init__(self):
        images_path = os.path.join(
            data_path, "test_pomBseen", "dual_channel", "Position_3", "Images"
        )
        intensity_image_path = os.path.join(
...
class Cdc42TimeLapseData(_Data):
    def __init__(self):
        images_path = os.path.join(
            data_path, "test_timelapse", "Kurt_ring", "Cdc42", "Position_1", "Images"
        )
        intensity_image_path = os.path.join(
...
class YeastMitoTimelapse(_Data):
    def __init__(self):
        ...
        acdc_df_path = os.path.join(
            images_path,
            "Point0019_ChannelGFP,mCardinal,Ph-3_Seq0019_s5_acdc_output.csv",
        )
        segm_path = os.path.join(
...
class BABYtestData(_Data):
    def __init__(self):
        images_path = os.path.join(
            data_path, "test_BABY", "evolve_testG_Brightfield", "Position_1", "Images"
        )
        intensity_image_path = os.path.join(images_path, "evolve_testG_Brightfield.tif")
        acdc_df_path = os.path.join(images_path, "evolve_testG_acdc_output.csv")
        segm_path = os.path.join(images_path, "evolve_testG_segm.npz")
...
class YeastMitoSnapshotData(_Data):
    def __init__(self):
        images_path = os.path.join(
            data_path, "test_snapshots", "mtDNA_Anika", "Position_10", "Images"
        )
        intensity_image_path = os.path.join(images_path, "ASY15-1_0nM-10_s10_mNeon.tif")
        acdc_df_path = os.path.join(images_path, "ASY15-1_0nM-10_s10_acdc_output.csv")
        segm_path = os.path.join(images_path, "ASY15-1_0nM-10_s10_segm.npz")
...
class MIA_KC_htb1_mCitrine(_Data):
    def __init__(self):
        ...
        segm_path = os.path.join(images_path, "19-03-2021_KCY050_SCGE_s02_segm.npz")
