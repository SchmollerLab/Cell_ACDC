...
class tracker:
    ...
    def track_baby_segm_data(self, segm_data, result_series, swap_YX_axes_to_XY):
        tracked_data = np.zeros_like(segm_data)
        result_generator = self.iterate_result_series(result_series, swap_YX_axes_to_XY)
        for frame_i, mask, ID in result_generator:
            tracked_data[frame_i][mask] = ID
        return tracked_data

    def track_external_segm_data(self, segm_data, result_series, swap_YX_axes_to_XY):
        result_generator = self.iterate_result_series(result_series, swap_YX_axes_to_XY)
        old_IDs_tracks = {}
        tracked_IDs_tracks = {}
