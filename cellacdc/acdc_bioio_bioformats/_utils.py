...
def getFilename(filenameNOext, s0p, appendTxt, series, ext, return_basename=False):
...
def saveImgDataChannel(
    ...
    if to_h5:
        filename = getFilename(filenameNOext, s0p, chName, series, ".h5")
        ...
        filename = getFilename(filenameNOext, s0p, chName, series, ".tif")
        imgData_ch = []
    ...
    pbar = tqdm(
        total=num_imgs, ncols=100, desc=f"Reading image (z 0/{SizeZ}, t 0/{numFrames})"
    )
...
def dump_exception(err, error_id):
    ...
    error_path = os.path.join(bioio_sample_data_folderpath, f"error_{error_id}.pkl")
