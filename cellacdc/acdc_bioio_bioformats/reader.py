...
def set_reader(image_filepath, **kwargs):
    ...
    if ext in EXTENSION_PACKAGE_MAPPER:
        all_kwargs = {**kwargs, **EXTENSION_BIOIMAGE_KWARGS_MAPPER.get(ext, {})}
        return all_kwargs
...
class ImageReader:
    def __init__(
        self, image_filepath: os.PathLike, qparent=None, lazy_load=True, **kwargs
    ):
        ...
        try:
            ...
            install.install_reader_dependencies(image_filepath, err, qparent=qparent)
            kwargs = set_reader(image_filepath, **kwargs)
            self._bioioimage = BioImage(image_filepath, **kwargs)
