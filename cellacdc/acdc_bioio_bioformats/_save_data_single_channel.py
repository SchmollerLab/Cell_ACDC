...
do_save_channels = [val=='True' for val in args['do_save_channels']]
...
zyx_physical_sizes = [float(val) for val in args['zyx_physical_sizes']]
PhysicalSizeZ, PhysicalSizeY, PhysicalSizeX = zyx_physical_sizes
to_h5 = args['to_h5']
timeRangeToSave = [int(val) for val in args['time_range_to_save']]
...
with bioformats.ImageReader(raw_filepath) as reader:
    bioformats._utils.saveImgDataChannel(
        ...
        channel_name, ch_idx, {}, SizeT, SizeZ, TimeIncrement, PhysicalSizeZ,
