...
class repeatDataPrepWindow(QDialog):
        ...
        if is_pos_folder:
            ...
            if len(values) > 1:
                ...
                if select_folder.cancel:
                    self.logger.info(
                        "Process aborted by the user (cancelled at Postion selection)"
                    )
                    self.stop()
                    return
    ...
    def selectChannels(self, ch_name_selector, ch_names, imagesPath, basename):
        if basename is not None:
            ...
            browseButton = widgets.browseFileButton(
                "Select .tif file to add and prep",
                start_dir=imagesPath,
                title="Select .tif file to add and prep",
                ext={"TIFF files": ".tif"},
            )
    ...
    def selectTifFileToAdd(self, tif_file_path):
        ...
        win = apps.filenameDialog(
            ...
            allowEmpty=False,
            defaultEntry=tif_filename,
            existingNames=self.ch_names,
            parent=self.selectChannelWindow,
        )
        win.exec_()
        if win.cancel:
            return
    ...
    def criticalNotValidFolder(self, path: os.PathLike):
        ...
        msg.critical(self, "Incompatible folder", txt, buttonsTexts=("Ok",))
