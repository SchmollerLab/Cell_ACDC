...
class CellCycle(UndoRedo):
    ...
    def _getCcaCostMatrix(self, numCellsG1, numNewCells, IDsCellsG1, newIDs_contours):
        ...
        if dist_matrix_df is None:
            ...
            for obj in posData.rp:
                ...
                for j, newID_cont in enumerate(newIDs_contours):
                    min_dist, nearest_xy = self.nearest_point_2Dyx(cont, newID_cont)
                    cost[i, j] = min_dist
    ...
    def addIDBaseCca_df(self, posData, ID):
        ...
        if posData.cca_df.empty:
            posData.cca_df = pd.DataFrame({col: val for col, val in _zip}, index=[ID])
        else:
            for col, val in _zip:
    ...
    def annotateBudToDifferentMother(self):
        ...
        if not self.isSnapshot:
            ...
            budEligible = self.checkChangeMotherBudEligible(budID, posData.frame_i)
            if not budEligible:
                return
        ...
        if posData.frame_i == 0:
            ...
            if not newMothCcs == "G1":
                err_msg = "You are assigning the bud to a cell that is not in G1!"
                msg = QMessageBox()
                msg.critical(self, "New mother not in G1!", err_msg, msg.Ok)
                return
    ...
    def annotateWillDivide(self, ID, relID, frame_i=None):
        ...
        for past_frame_i in range(frame_i - 1, -1, -1):
            ...
            self.store_cca_df(cca_df=past_cca_df, frame_i=past_frame_i, autosave=False)
    ...
    def autoAssignBud_YeastMate(self):
        if not self.is_win:
            ...
            msg = QMessageBox()
            msg.critical(self, "Supported only on Windows", txt, msg.Ok)
            return
    ...
    def autoCca_df(self, enforceAll=False):
        ...
        isLastVisitedAgain = self.isLastVisitedAgainCca(curr_df, enforceAll=enforceAll)

        frameAlreadyAnnotated = (
            posData.cca_df is not None and not enforceAll and not isLastVisitedAgain
        )
        ...
        if isLastVisitedAgain and not enforceAll:
            try:
                correctedAssignIDs = curr_df[curr_df["corrected_on_frame_i"] > 0].index
            except Exception as e:
                correctedAssignIDs = []
            posData.new_IDs = [
        ...
        if isLastVisitedAgain or enforceAll:
            ...
            new_cell_G1 = [ID for ID in current_G1_IDs if ID not in prev_cca_df.index]
        ...
        if numCellsG1 < numNewCells:
            notEnoughG1Cells, proceed = self.handleNoCellsInG1(numCellsG1, numNewCells)
            return notEnoughG1Cells, proceed
    ...
    def ccaIntegrCheckerToggled(self, checked):
        self.df_settings.at["is_cca_integrity_checker_activated", "value"] = int(
            checked
        )
    ...
    def checkMothersExcludedOrDead(self):
        try:
            ...
            print("-" * 100)
            self.logger.warning("Checking if mother cell is excluded or dead failed.")
            print("^" * 100)
            return False
    ...
    def checkSwapMothersEligibility(self):
        ...
        for _budID in (budID, otherBudID):
            result = self._checkBudFutureNoDivision(_budID, posData.frame_i)
            if result is None:
                continue
        ...
        correct_pairings = {otherBudID: mothID, budID: otherMothID}
        wrong_pairings = {mothID: budID, otherMothID: otherBudID}
    ...
    def fixWillDivide(self, warning_txt, IDs_will_divide_wrong):
        ...
        global_cca_df = global_cca_df.reset_index().set_index(["frame_i", "Cell_ID"])
    ...
    def highlightNewCellNotEnoughG1cells(self, IDsCellsG1):
        ...
        for obj in posData.rp:
            ...
            self.textAnnot[0].addObjAnnotation(obj, "green", f"{obj.label}?", False)
    ...
    def initCca(self):
        ...
        if last_tracked_i == 0:
            ...
            msg.critical(self, "Tracking was never checked", txt)
        ...
        if posData.frame_i > last_cca_frame_i:
            ...
            _, goToFrameButton, stayButton = msg.warning(
                self,
                "Go to last annotated frame?",
                txt,
                buttonsTexts=(
                    "Cancel",
                    f"Yes, go to frame {last_cca_frame_i + 1}",
    ...
    def manualEditCca(self, checked=True):
        ...
        editCcaWidget = apps.editCcaTableWidget(
            posData.cca_df, posData.SizeT, current_frame_i=posData.frame_i, parent=self
        )
    ...
    def reInitCca(self):
        if not self.isSnapshot:
            ...
            msg = widgets.myMessageBox()
            msg.warning(
                self, "Re-initialize annnotations?", txt, buttonsTexts=("Cancel", "Yes")
            )
            posData = self.data[self.pos_i]
            if msg.cancel:
                return
    ...
    def repeatAutoCca(self):
        ...
        if next_df is not None:
            if "cell_cycle_stage" in next_df.columns:
                ...
                warn_cca = msg.critical(
                    self,
                    "Future visited frames detected!",
        ...
        correctedAssignIDs = posData.cca_df[
            posData.cca_df["corrected_on_frame_i"] >= 0
        ].index
    ...
    def resetFutureCcaColCurrentFrame(self):
        ...
        mothers_mask = (posData.cca_df.relationship == "mother") & cca_df_S_mask
        ...
        if cca_df is not None:
            ...
            mothers_mask = (cca_df.relationship == "mother") & cca_df_S_mask
    ...
    def setCcaIssueContour(self, obj):
        ...
        self.textAnnot[0].addObjAnnotation(obj, "lost_object", f"{obj.label}?", False)
    ...
    def updateCcaDfDeletedIDsTimelapse(
        self, posData, relIDsOfDelIDs, deletedIDs, undoId, dropInPast, dropInFuture
    ):
        ...
        for fut_frame_i in range(posData.frame_i + 1, posData.SizeT):
            ...
            self.store_cca_df(frame_i=fut_frame_i, cca_df=cca_df_i, autosave=False)
        ...
        for past_frame_i in range(posData.frame_i - 1, -1, -1):
            ...
            self.store_cca_df(frame_i=past_frame_i, cca_df=cca_df_i, autosave=False)
    ...
    def viewCcaTable(self):
        ...
        df = posData.add_tree_cols_to_cca_df(current_cca_df, frame_i=posData.frame_i)
        if self.ccaTableWin is None:
            ...
            self.ccaTableWin.sigUpdateCcaTable.connect(self.onSigUpdateCcaTableWindow)
        else:
    ...
    def warnBudAnnotatedDividedInFuture(
        self, budID, motherID, future_division_frame_i, action="swap mother cells"
    ):
        posData = self.data[self.pos_i]
    ...
    def warnDeadOrExcludedMothers(self, budIDs, mothIDs):
        ...
        msg = widgets.myMessageBox(wrapText=False)
        pairings = [
            f"Mother ID {mID} --> bud ID {bID}" for mID, bID in zip(mothIDs, budIDs)
        ]
        txt = html_utils.paragraph(f"""
        ...
        """)
        msg.warning(
            self, "Mother cell is excluded or dead", txt, buttonsTexts=("Cancel", "Ok")
        )
    ...
    def warnEditingWithCca_df(
        ...
        _, removeAnnotButton, _ = msg.warning(
            self,
            "Edited segmentation with annotations!",
            txt,
            buttonsTexts=(
                "Cancel",
        ...
        if (msg.clickedButton == removeAnnotButton) and cell_cycle_stage_present:
            ...
            if dropDelIDsNoteText and posData.cca_df is not None:
                ...
                self.update_cca_df_deletedIDs(posData, delIDs, dropInPast=False)
            self.addMissingIDs_cca_df(posData)
            self.updateAllImages()
            self.store_data()
        # if action is not None:
    ...
    def warnMotherNotEligible(self, new_mothID, budID, i, why):
        if why == "not_G1_in_the_future":
            ...
            _, applyButton = msg.warning(
                self, "Cell not eligible", err_msg, buttonsTexts=("Cancel", applyButton)
            )
            cancel = msg.cancel
            ...
            """)
            msg = widgets.myMessageBox()
            msg.warning(self, "Cell not eligible", err_msg)
            cancel = msg.cancel
            apply = False
    ...
    def warnSettingHistoryKnownCellsFirstFrame(self, ID):
        ...
        """)
        msg = widgets.myMessageBox(wrapText=False)
        msg.warning(self, "First frame cells", txt)
    ...
    def annotateDivisionFutureFramesSwapMothers(
        ...
        cca_df_at_future_division.at[mothIDofDisappearedBud, "corrected_on_frame_i"] = (
            frame_i
        )
        self.store_cca_df(
    ...
    def getStatus_RelID_BeforeEmergence(self, budID, curr_mothID):
        ...
        cca_status_before_bud_emerg = core.getBaseCca_df([curr_mothID]).loc[curr_mothID]
        return cca_status_before_bud_emerg
    ...
    def restoreMotherToBeforeWrongBudWasAssignedToIt(
        self, mothIDofDisappearedBud, cca_df_at_correct_bud_ID_disappearance, frame_i
    ):
        """This method is called as part of `guiWin.swapMothers`.
    ...
    def annotateDisappearedBeforeDivision(self, relID, IDgone, cca_df, frame_i=None):
        posData = self.data[self.pos_i]
