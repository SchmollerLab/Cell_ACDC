...
from cellacdc import core, workers, widgets, html_utils, apps, preprocess, myutils
...
class CombineMixin:
    ...
    def combineDialogSaveCombinedData(self, dialog):
        ...
        self.saveCombinedChannelsWorker.signals.critical.connect(self.workerCritical)
        ...
        self.saveCombinedChannelsWorker.signals.progress.connect(self.workerProgress)
        self.saveCombinedChannelsWorker.signals.finished.connect(
    ...
    def combineDialogStepsChanged(self):
        ...
        self.updateCombineChannelsPreview(
            steps=steps, keep_input_data_type=keep_input_data_type, formula=formula
        )
    ...
    def combineViewAsSegmSetup(self):
        ...
        if not self.overlayLabelsButton.isChecked() and combineViewAsSegm:
            ...
            self.overlayLabels_cb(
                checked=True, selectedLabelsEndnames=["combined segm."]
            )
    ...
    def combineWorkerDone(
        self, processed_data: List[np.ndarray], keys: List[Tuple[int, int, int]]
    ):
    ...
    def setupCombiningChannels(self):
        ...
        self.combineDialog.sigApplyImage.connect(self.combineCurrentImage)
        self.combineDialog.sigApplyZstack.connect(self.combineZStack)
        self.combineDialog.sigApplyAllFrames.connect(self.combineAllFrames)
        self.combineDialog.sigApplyAllPos.connect(self.combineAllPos)
        self.combineDialog.sigPreviewToggled.connect(self.combinePreviewToggled)
        self.combineDialog.sigSaveAsSegmCheckboxToggled.connect(
            self.combinePreviewViewAsSegmToggled
        )
        self.combineDialog.sigValuesChanged.connect(self.combineDialogStepsChanged)
        self.combineDialog.sigSavePreprocData.connect(
            self.combineDialogSaveCombinedData
        )
        self.combineDialog.sigClose.connect(self.combineDialogClosed)
        ...
        self.combineWorker.signals.finished.connect(self.combineWorker.deleteLater)
        ...
        self.combineWorker.sigIsQueueEmpty.connect(self.combineWorkerIsQueueEmpty)
        ...
        self.combineWorker.sigAskLoadChannels.connect(self.combineWorkerAskLoadChannels)
    ...
    def update_combine_image_data(
        self, pos_data, pos_i_data: list[tuple[tuple[int, int, int], np.ndarray]]
    ):
