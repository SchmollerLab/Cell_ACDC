...
class Saving(AppShell):
    ...
    def _enqueueAutoSave(self):
        if not self.statusBarLabel.text().endswith("Autosaving..."):
            self.statusBarLabel.setText(f"{self.statusBarLabel.text()} | Autosaving...")
    ...
    def askConcatenate(self):
        ...
        showAskConcatenate = self.df_settings.at["showAskConcatenate", "value"] == "Yes"
        ...
        showAskConcatenate = "No" if doNotShowAgainCheckbox.isChecked() else "Yes"
        self.df_settings.at["showAskConcatenate", "value"] = showAskConcatenate
    ...
    def askSaveLastVisitedCcaMode(self, isQuickSave=False):
        ...
        lastFrameDialog = apps.QLineEditDialog(
            ...
            msg=txt,
            parent=self,
            allowedValues=(1, last_cca_frame_i + 1),
            warnLastFrame=True,
            isInteger=True,
            stretchEntry=False,
    ...
    def askSaveLastVisitedSegmMode(self, isQuickSave=False):
        ...
        """)
        lastFrameDialog = apps.QLineEditDialog(
            title="Last frame number to save",
            defaultTxt=str(frame_i + 1),
            msg=txt,
            parent=self,
            allowedValues=(1, posData.SizeT),
            warnLastFrame=True,
            isInteger=True,
            stretchEntry=False,
    ...
    def askSaveOriginalSegm(self, isQuickSave=False):
        ...
        existingEndnames = load.get_endnames(posData.basename, found_files)
    ...
    def autoSaveIntervalValueChanged(
        ...
        self.logger.info(f"Autosave interval changed to: {value} {unit}")
    ...
    def autoSaveTimerCountFrames(self):
        ...
        posData = self.data[self.pos_i]
        autoSaveIntevalValue, autoSaveIntervalUnit = self.autoSaveIntevalValueUnit
        isTimeToAutoSave = (
    ...
    def cancelSavingInitialisation(self):
        self.titleLabel.setText("Saving data process cancelled.", color=self.titleColor)
        self.closeGUI = False
    ...
    def saveAsData(self, checked=True):
        ...
        for _posData in self.data:
            ...
            existingFilenames.update(
                [f"{_posData.basename}{endname}.npz" for endname in _existingEndnames]
            )
        posData = self.data[self.pos_i]
    ...
    def saveData(self, checked=False, finishedCallback=None, isQuickSave=False):
        ...
        self.worker.customMetricsCritical.connect(self.saveDataCustomMetricsCritical)
        ...
        self.worker.regionPropsCritical.connect(self.saveDataRegionPropsCritical)
    ...
    def saveDataUpdateMetricsPbar(self, max, step):
        ...
        self.saveWin.metricsQPbar.setValue(self.saveWin.metricsQPbar.value() + step)
    ...
    def warnErrorsCustomMetrics(self):
        win = apps.ComputeMetricsErrorsDialog(
            self.worker.customMetricsErrors,
            self.logs_path,
    ...
    def warnErrorsRegionProps(self):
        win = apps.ComputeMetricsErrorsDialog(
            self.worker.regionPropsErrors,
            self.logs_path,
            log_type="region_props",
            parent=self,
        )
        win.exec_()
