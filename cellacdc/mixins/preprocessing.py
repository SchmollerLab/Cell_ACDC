...
class Preprocessing(Session):
    ...
    def preprocWorkerDone(
        ...
        if how == "current_image":
            if posData.SizeZ > 1:
                ...
                posData.preproc_img_data[posData.frame_i][z_slice] = processed_data
            else:
            ...
            for z_slice, processed_img in enumerate(processed_data):
                posData.preproc_img_data[posData.frame_i][z_slice] = processed_img
                self.img1.updateMinMaxValuesPreprocessedData(
                    self.data, self.pos_i, posData.frame_i, z_slice
                )
            ...
            for frame_i, processed_frame in enumerate(processed_data):
                ...
                for z_slice, processed_img in enumerate(processed_frame):
                    posData.preproc_img_data[frame_i][z_slice] = processed_img
                    self.img1.updateMinMaxValuesPreprocessedData(
            ...
            for pos_i, processed_pos_data in enumerate(processed_data):
                ...
                for z_slice, processed_img in enumerate(processed_pos_data):
                    posData.preproc_img_data[0][z_slice] = processed_img
                    self.img1.updateMinMaxValuesPreprocessedData(
    ...
    def preprocWorkerPreviewDone(
        self, processed_data: np.ndarray, key: Tuple[int, int, Union[int, str]]
    ):
        ...
        self.img1.updateMinMaxValuesPreprocessedData(self.data, pos_i, frame_i, z_slice)
    ...
    def preprocessAllFrames(self, recipe: List[Dict[str, Any]]):
        ...
        self.preprocWorker.setupJob(func, image_data, recipe, "all_frames")
    ...
    def preprocessAllPos(self, recipe: List[Dict[str, Any]]):
        ...
        self.preprocWorker.setupJob(func, image_data, recipe, "all_pos")
    ...
    def preprocessCurrentImage(self, recipe: List[Dict[str, Any]], *args):
        ...
        self.preprocWorker.setupJob(func, image_data, recipe, "current_image")

        self.preprocWorker.wakeUp()

    def preprocessDialogRecipeChanged(
        self, recipe
    ):  # why does this need the recepie as an arg
    ...
    def preprocessDialogSavePreprocessedData(self, dialog):
        ...
        self.savePreprocWorker.signals.finished.connect(self.savePreprocThread.quit)
        self.savePreprocWorker.signals.finished.connect(
            self.savePreprocWorker.deleteLater
        )
        self.savePreprocThread.finished.connect(self.savePreprocThread.deleteLater)

        self.savePreprocWorker.signals.critical.connect(self.workerCritical)
        self.savePreprocWorker.signals.initProgressBar.connect(
            self.workerInitProgressbar
        )
        self.savePreprocWorker.signals.progressBar.connect(self.workerUpdateProgressbar)
        self.savePreprocWorker.signals.progress.connect(self.workerProgress)
        self.savePreprocWorker.signals.finished.connect(self.savePreprocWorkerFinished)

        self.savePreprocThread.started.connect(self.savePreprocWorker.run)
    ...
    def preprocessEnqueueCurrentImage(self, recipe):
        ...
        self.preprocWorker.enqueue(func, image_data, recipe, key)
    ...
    def preprocessZStack(self, recipe: List[Dict[str, Any]], *args):
        ...
        self.preprocWorker.setupJob(func, image_data, recipe, "z_stack")
    ...
    def setupPreprocessing(self):
        ...
        self.preprocessDialog.sigApplyImage.connect(self.preprocessCurrentImage)
        self.preprocessDialog.sigApplyZstack.connect(self.preprocessZStack)
        self.preprocessDialog.sigApplyAllFrames.connect(self.preprocessAllFrames)
        self.preprocessDialog.sigApplyAllPos.connect(self.preprocessAllPos)
        self.preprocessDialog.sigPreviewToggled.connect(self.preprocessPreviewToggled)
        ...
        self.preprocWorker.signals.finished.connect(self.preprocWorker.deleteLater)
        ...
        self.preprocWorker.sigIsQueueEmpty.connect(self.preprocWorkerIsQueueEmpty)
