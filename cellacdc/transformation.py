...
def trackmate_xml_to_df(xml_file):
    ...
    df = pd.DataFrame({"frame_i": frame_idxs, "ID": IDs, "x": xx, "y": yy, "z": zz})
    return df
...
def retrack_based_on_untracked_first_frame(
    ...
    untracked_to_unique_mapper = dict(zip(first_untracked_IDs, uniqueIDs))
    ...
    for frame_i, tracked_lab in enumerate(tracked_video):
        ...
        for obj_tracked in rp_tracked:
            ...
            if untracked_obj is not None:
                ...
                lab[untracked_obj.slice][untracked_obj.image] = new_unique_ID

            # Replace tracked ID of first frame to the untracked ID of the
            # reference
            tracked_video[frame_i][obj_tracked.slice][obj_tracked.image] = untracked_ID
        pbar.update()
    pbar.close()
...
def crop_outer_padding(arr, value=0, copy=False):
    ...
    is_padded = is_top_padded or is_bottom_padded or is_left_padded or is_right_padded

    if not is_padded:
        return arr.copy() if copy else arr

    # Crop using regionprops
    padding_mask_rp = skimage.measure.regionprops(skimage.measure.label(~padding_mask))
