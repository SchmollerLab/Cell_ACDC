...
def shiftingstuff_other(
    tif_name, shift, tif_path, scan_other, start_frame, end_frame, NEW_PATH_SUF
):
    if scan_other == True:
        tif_path = os.path.join(os.path.dirname(tif_path), tif_name)
        tif_data = load.imread(tif_path)
        shiftingstuff_main(
            shift, tif_data, tif_path, start_frame, end_frame, NEW_PATH_SUF
        )
        del tif_data
    return
...
def sequential(NEW_PATH_SUF):
    ...
    while True:
        answer = input(
            "Do you want to shift the other .tif files in the folder too? ([y]/n/help)"
        )
        if answer.lower() == "n":
            scan_other = False
            break
...
if __name__ == "__main__":
    ...
    shift, tif_data, tif_names, scan_other, tif_path, start_frame, end_frame = (
        sequential(NEW_PATH_SUF)
    )
