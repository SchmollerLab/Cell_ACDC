...
def select_segm_masks(exp_path, pos_foldernames):
    ...
    win = apps.SelectSegmFileDialog(
        existingEndNames, exp_path, infoText=infoText, fileType="segmentation"
    )
    win.exec_()
    if win.cancel:
        return
    ...
    for pos in pos_foldernames:
        ...
        for segm_endname in list_segm_endnames_to_split:
            segm_data_to_split, segm_data_to_split_fp = load.load_segm_file(
                images_path, end_name_segm_file=segm_endname, return_path=True
            )
            out = core.split_segm_masks_mother_bud_line(
                cells_segm_data, segm_data_to_split, acdc_df, debug=DEBUG
            )
            ...
            split_close_filepath = os.path.join(images_path, split_close_filename)
            ...
            split_away_filepath = os.path.join(images_path, split_away_filename)
