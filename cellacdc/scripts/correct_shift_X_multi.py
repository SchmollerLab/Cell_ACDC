...
def finding_base_tif_files_path(root_path):
    ...
    folder_list = [
        os.path.join(root_path, folder_name, "Images")
        for folder_name in folder_list
        if folder_name.lower().startswith(FOLDER_FILTER.lower())
    ]
    for folder_name in folder_list:
...
def find_other_tif(file_path):
    ...
    file_list = [
        filename for filename in file_list if filename.lower().endswith(".tif")
    ]
    return file_list
...
def finding_shift(tif_data, shift, NEW_PATH_SUF):
    ...
    while True:
        answer = input(
            'Do you want to proceed with the shift or change it?([y]/n/"number"/help)'
        )
        if answer.lower() == "n":
            exit()
        elif answer.isdigit():
            ...
            print(
                "Change the shown image by changing PREVIEW_Z_STACK and PREVIEW_Z in the beginning of the code. \nChange the ending of the new file name by changing NEW_PATH_SUF in the code. \nCurrent z stack and z displayed: "
                + str(PREVIEW_Z_STACK)
                + " "
                + str(PREVIEW_Z)
                + "\nCurrent ending: "
...
def sequential(NEW_PATH_SUF):
    ...
    parser.add_argument(
        "root_path",
        help="Path to the folder containing all the folders with the positions",
    )
    ...
    while True:
        answer = input(
            "Do you want to shift the other .tif files in the folders too? ([y]/n/help)"
        )
        if answer.lower() == "n":
            scan_other = False
            break
    ...
    for i, tif_path in enumerate(base_file_paths):
        ...
        if scan_other == True:
            ...
            other_tif_files = [
                tif_file
                for tif_file in other_tif_files
                if re.match(INCLUDE_PATTERN_TIF_SEARCH, tif_file)
            ]
            other_tif_files = [
                os.path.join(other_files_paths[i], tif_file)
                for tif_file in other_tif_files
            ]
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        futures = [
            executor.submit(shiftingstuff_other, shifttif, NEW_PATH_SUF)
            for shifttif in tif_files_master
        ]
