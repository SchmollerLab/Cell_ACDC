import os, traceback, cv2, subprocess, re
...
class manual_emerg_bud:
    def __init__(self, label_img, ID, rp, eps_percent=0.01, del_small_obj=False):
        ...
        cnt, defects = self.convexity_defects(self.only_ID_img.astype(np.uint8),
                                              eps_percent)
        ...
        self.undo_B = Button(self.ax_undo_B, 'Undo', canvas=sub_win.canvas,
                            color='0.1', hovercolor='0.25', presscolor='0.35')
        """Connect to events"""
        (sub_win.canvas).mpl_connect('button_press_event', self.press)
        (sub_win.canvas).mpl_connect('button_release_event', self.release)
