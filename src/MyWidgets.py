...
class Button(AxesWidget):
    ...
    def __init__(self, ax, label, image=None, canvas=None,
                 color='0.85', hovercolor='0.95',
                 size=10, x=0.5, presscolor='1'):
        """
        Parameters
        ----------
        ...
        if image is not None:
            ax.imshow(image)
        self.label = ax.text(x, 0.5, label,
    ...
    def _click(self, event):
        ...
        if event.inaxes != self.ax:
            ...
            c = self.presscolor
            if c != self._lastcolorclick:
                self.ax.set_facecolor(c)
                self.__lastcolorclick = c
                if self.drawon:
...
class Slider(AxesWidget):
    ...
    val : float
        Slider value.
    """
    def __init__(self, ax, label, valmin, valmax, canvas=None,
                 valinit=0.5, valfmt='%1.2f', closedmin=True,
                 closedmax=True, slidermin=None, slidermax=None,
                 dragging=True, valstep=None, orientation='horizontal',
                 init_val_line=None, init_val_line_color='r', **kwargs):
        """
        Parameters
        ----------
        ax : Axes
        ...
        self.halfwidth = (valstep/2)
        box1 = valinit - (valstep/2)
        box2 = valinit + (valstep/2)
        self.box1 = box1
        ...
        self.init_val_line_color = init_val_line_color
        valinitline = init_val_line if init_val_line else valinit
        if orientation == 'vertical':
            self.poly = ax.axhspan(valmin, box2, 0, 1, **kwargs)
            self.hline = ax.axhline(valinitline, 0, 1,
                                    color=init_val_line_color, lw=1)
        else:
            self.poly = ax.axvspan(box1, box2, 0, 1, **kwargs)
            self.vline = ax.axvline(valinitline, 0, 1,
                                    color=init_val_line_color, lw=1)
        self.valfmt = valfmt
        ax.set_yticks([])
        if orientation == 'vertical':
    ...
    def set_val(self, val, silent=False, reset_line=False):
        """
        ...
        if self.orientation == 'vertical':
            # xy[1] = 0, val
            # xy[2] = 1, val
            xy[2] = 1, val + self.halfwidth
            xy[3] = 0, val + self.halfwidth
            xy[0] = 0, val - self.halfwidth
            xy[1] = 1, val - self.halfwidth
            xy[4] = 0, val - self.halfwidth
        else:
            xy[2] = val + self.halfwidth, 1
            xy[3] = val + self.halfwidth, 0
            xy[0] = val - self.halfwidth, 0
            xy[1] = val - self.halfwidth, 1
            xy[4] = val - self.halfwidth, 0
        self.poly.xy = xy
        ...
        if reset_line:
            ...
            if self.orientation == 'vertical':
                ...
                self.vline.remove()
                self.vline = ax.axvline(val, 0, 1,
                                        color=self.init_val_line_color, lw=1)
...
class RadioButtons(AxesWidget):
    ...
    """
    def __init__(self, ax, labels, active=0, activecolor='blue', canvas=None):
        """
        ...
        activecolor : color
            The color of the selected button.
        """
        AxesWidget.__init__(self, ax, canvas=canvas)
...
class MyRadioButtons(RadioButtons):
    def __init__(self, ax, labels, active=0, activecolor='blue', size=49,
                 orientation="vertical", circ_p_color='black', canvas=None,
                 **kwargs):
        """
        Add radio buttons to an `~.axes.Axes`.
        Parameters
        ...
        circ_p_color : color
            The color of button's edge (circle perimeter)
        ...
        for i, label in enumerate(labels):
            if i == active:
                ...
                facecolor = axcolor
            p = ax.scatter([],[], s=size, marker="o", edgecolor=circ_p_color,
                           facecolor=facecolor)
            circles.append(p)
